/* frarik-version: 3.2 */
/**
 * GruppoLuci.js — Distintivo FratechStore v3.2
 * v3.2: righe entità del popup config a stile "glass" (badge icona rotondo,
 *       nome/entity-id chiaramente distinti, automazione come pillola blu
 *       separata invece di box incollato) — meno "ammassato", più leggibile
 ...
 *       (12px) invece di più livelli diversi — richiesta esplicita di uniformità assoluta
...
(function () {
  ...
  function render(cfg, rawHass, overrides) {
      ...
      <div style="display:flex;gap:20px;justify-content:center;margin:0 14px 16px">
        <button data-gl-all="on" style="background:none;border:none;padding:4px 2px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:7px;outline:none">
          <span style="width:48px;height:48px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:linear-gradient(155deg,${hex2rgba('#4ade80',.3)},${hex2rgba('#4ade80',.08)});border:1.5px solid ${hex2rgba('#4ade80',.55)};box-shadow:0 0 16px ${hex2rgba('#4ade80',.35)}">
            <span style="font-size:21px;color:#4ade80">${iconHtml('mdi:lightbulb-on', 21)}</span>
          </span>
          <span style="white-space:nowrap;color:#4ade80;text-transform:uppercase;font-weight:900;font-size:12px">Accendi tutte</span>
        </button>
    ...
    const rows = ents.map((e, i) => {
      ...
      const tile = `<div data-jsd-toggle="${i}" style="position:relative;overflow:hidden;flex:1;min-width:0;max-width:calc(100% - 78px);display:flex;align-items:center;gap:13px;border-radius:18px;cursor:pointer;background:linear-gradient(155deg,${on?hex2rgba(col,.18):hex2rgba('#ffffff',.05)},${on?hex2rgba(col,.03):hex2rgba('#ffffff',.01)});border:1px solid rgba(255,255,255,.25);padding:16px 16px">
        ...
        <span style="position:relative;flex:1;min-width:0">
          ...
          <span style="display:block;font-size:12px;font-weight:900;text-transform:uppercase;color:${on?col:'#fff'};letter-spacing:.3px;margin-top:2px">${on?'Accesa':'Spenta'}</span>
        </span>
      </div>`;
    ...
    return `<div id="gl-popup-body">
      ${hero}
      ${ctrlBar}
      <div>${rows||'<div style="padding:32px 20px;text-align:center;color:#fff;font-size:12px;font-weight:900;text-transform:uppercase">Nessuna luce configurata<br><span style="font-size:12px;font-weight:900;text-transform:uppercase;opacity:.7">Clicca ✏️ sulla chip per configurare</span></div>'}</div>
    </div>`;
  }
  ...
  function configure(cfg, _el, onSave) {
    ...
    function renderForm() {
      ...
      const selRows = ents.map((e, i) => {
        ...
        // riga automazione: sempre visivamente separata dalla riga entità (sfondo/bordo blu dedicato)
        ...
        return `<div style="position:relative;overflow:hidden;padding:12px;border-radius:14px;background:linear-gradient(155deg,${hex2rgba(rc,on?.16:.06)},${hex2rgba(rc,on?.03:.02)});border:1px solid ${hex2rgba(rc,on?.35:.16)};margin-bottom:10px">
          ...
          <div style="position:relative;display:flex;align-items:center;gap:10px">
            <div style="width:32px;height:32px;flex-shrink:0;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:16px;background:${hex2rgba(rc,on?.22:.08)};border:1px solid ${hex2rgba(rc,on?.4:.16)};filter:${on?'none':'grayscale(1) opacity(.5)'}">💡</div>
            <div style="flex:1;min-width:0">
  ...
  try { console.log('[FratechStore] Distintivo registrato: gruppo-luci v3.2'); } catch(e){}
})();
