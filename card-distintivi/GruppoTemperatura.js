/* frarik-version: 1.1 */
/**
 * GruppoTemperatura.js — Distintivo FratechStore v1.1
 * Chip usa sensori di media; popup mostra singoli sensori per stanza
 * Testi sempre #fff, colori dinamici solo sui valori numerici
 */
(function () {
  'use strict';
  ...
  function chip(cfg, rawHass) {
    ...
    if (h) {
      if (c.avgTempEntity) {
        /* usa sensore di media configurato */
        ...
        /* fallback: calcola min-max dai sensori singoli */
        ...
        <div style="margin-bottom:14px;padding:16px;border-radius:18px;background:rgba(255,255,255,.04);border:1px solid ${unavail ? 'rgba(255,255,255,.08)' : hex2rgba(tCol,.22)};box-shadow:0 4px 24px ${unavail ? 'transparent' : hex2rgba(tCol,.08)}">
          ...
          <div style="display:flex;align-items:center;gap:9px;margin-bottom:14px">
            ...
            ${!unavail
              ...
              : `<span style="font-size:10px;color:#fff;font-style:italic">Non disponibile</span>`
            }
          </div>
          ...
          <div style="display:flex;gap:10px">
            ...
            <div style="flex:1;padding:13px 14px 11px;border-radius:14px;background:${hex2rgba(tCol,.08)};border:1px solid ${hex2rgba(tCol,.2)}">
              <div style="font-size:10px;color:#fff;margin-bottom:6px;display:flex;align-items:center;gap:5px;font-weight:600;letter-spacing:.3px">
                <span class="mdi mdi-thermometer" style="font-size:14px;color:${tCol}"></span>
                TEMPERATURA
              </div>
              <div style="display:flex;align-items:baseline;gap:3px;margin-bottom:10px">
                ...
                <span style="font-size:17px;font-weight:700;color:#fff">°C</span>
              </div>
              ...
              <div style="display:flex;justify-content:space-between;margin-top:5px">
                <span style="font-size:9px;color:#fff">❄️ 0°</span>
                <span style="font-size:9px;color:#fff">🔥 40°</span>
              </div>
            </div>
            ...
            ${hasHum ? `<div style="flex:1;padding:13px 14px 11px;border-radius:14px;background:${hex2rgba(hCol,.08)};border:1px solid ${hex2rgba(hCol,.2)}">
              <div style="font-size:10px;color:#fff;margin-bottom:6px;display:flex;align-items:center;gap:5px;font-weight:600;letter-spacing:.3px">
                <span class="mdi mdi-water-percent" style="font-size:14px;color:${hCol}"></span>
                UMIDITÀ
              </div>
              <div style="display:flex;align-items:baseline;gap:3px;margin-bottom:10px">
                <span style="font-size:34px;font-weight:900;color:${hCol};line-height:1;letter-spacing:-1.5px">${humStr}</span>
                <span style="font-size:17px;font-weight:700;color:#fff">%</span>
              </div>
              <div style="height:4px;border-radius:2px;background:rgba(255,255,255,.1);overflow:hidden">
                <div style="width:${hPct}%;height:100%;border-radius:2px;background:${hCol};transition:width .5s ease"></div>
              </div>
              <div style="display:flex;justify-content:space-between;margin-top:5px">
                <span style="font-size:9px;color:#fff">🏜 0%</span>
                <span style="font-size:9px;color:#fff">💦 100%</span>
              </div>
            </div>` : ''}
  ...
  function configure(cfg, _el, onSave) {
    ...
    function renderForm() {
      ...
      return `<div style="width:100%;max-height:92vh;display:flex;flex-direction:column;background:#0b1422;border:1px solid rgba(56,189,248,.22);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -16px 60px rgba(0,0,0,.9);color:#fff;${anim}">
        ...
        <div id="gtcfg-body" style="flex:1;overflow-y:auto;overflow-x:hidden;scrollbar-width:none;padding:14px 14px 4px">
          ...
          <!-- Sensori per stanza -->
          ${ents.length ? `
            <div class="gtsec">Sensori per stanza (${ents.length})</div>
            ${selRows}
          ` : ''}
    ...
    function attach() {
      ...
      /* autocomplete sui sensori di media */
      ...
      /* autocomplete sui sensori per stanza */
  ...
  const CARD = {
    ...
    icon: '🌡️',
    desc: 'Chip con media temp/umidità; popup con sensori per stanza, colori dinamici e badge comfort.',
    version: '1.1',
    isDistintivo: true,
  ...
  try { console.log('[FratechStore] Distintivo registrato: gruppo-temperatura v1.1'); } catch(e) {}
})();
