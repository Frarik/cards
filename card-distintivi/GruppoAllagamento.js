/* frarik-version: 1.0 */
/**
 * GruppoAllagamento.js — Distintivo FratechStore v1.0
 * Chip sensori allagamento/umidità — verde se tutto asciutto, rosso se qualcuno è bagnato
 */
(function () {
  'use strict';
  ...
  function _floodSvg(isWet, idx) {
    const sid = `gpfl${idx}`;
    if (!isWet) {
      return `<svg viewBox="0 0 52 72" width="52" height="72" style="display:block;overflow:visible">
        <path d="M26,8 C26,8 40,32 40,46 A14,14 0 0,1 12,46 C12,32 26,8 26,8Z" fill="rgba(56,189,248,.15)" stroke="rgba(56,189,248,.3)" stroke-width="1.5"/>
        <path d="M26,22 C26,22 34,40 34,47 A8,8 0 0,1 18,47 C18,40 26,22 26,22Z" fill="rgba(56,189,248,.12)"/>
        <text x="26" y="70" text-anchor="middle" font-size="9" font-weight="700" fill="rgba(74,222,128,.8)" font-family="system-ui">OK</text>
      </svg>`;
    }
    return `<style>
      @keyframes ${sid}Dw{0%{transform:translateY(0);opacity:1}100%{transform:translateY(12px);opacity:0}}
      @keyframes ${sid}Wv{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}
      @keyframes ${sid}Pu{0%,100%{opacity:.6}50%{opacity:1}}
    </style>
    <svg viewBox="0 0 52 72" width="52" height="72" style="display:block;overflow:visible">
      <path d="M26,8 C26,8 40,32 40,46 A14,14 0 0,1 12,46 C12,32 26,8 26,8Z" fill="rgba(248,113,113,.28)" stroke="rgba(248,113,113,.65)" stroke-width="1.5" style="animation:${sid}Pu 1.4s ease-in-out infinite"/>
      <path d="M26,22 C26,22 34,40 34,47 A8,8 0 0,1 18,47 C18,40 26,22 26,22Z" fill="rgba(248,113,113,.45)"/>
      <clipPath id="${sid}clip"><rect x="4" y="54" width="44" height="14" rx="3"/></clipPath>
      <rect x="4" y="54" width="44" height="14" rx="3" fill="rgba(56,189,248,.18)"/>
      <g clip-path="url(#${sid}clip)">
        <path d="M-4,60 Q8,56 20,60 Q32,64 44,60 Q56,56 68,60" stroke="rgba(56,189,248,.7)" stroke-width="2" fill="none" style="animation:${sid}Wv 1.8s linear infinite"/>
        <path d="M-4,65 Q8,61 20,65 Q32,69 44,65 Q56,61 68,65" stroke="rgba(56,189,248,.4)" stroke-width="2" fill="none" style="animation:${sid}Wv 1.8s linear .6s infinite"/>
      </g>
      <circle cx="20" cy="42" r="2" fill="rgba(56,189,248,.9)" style="animation:${sid}Dw 1.2s ease-in infinite"/>
      <circle cx="32" cy="40" r="2" fill="rgba(56,189,248,.9)" style="animation:${sid}Dw 1.2s ease-in .5s infinite"/>
      <text x="26" y="72" text-anchor="middle" font-size="9" font-weight="800" fill="#f87171" font-family="system-ui">!</text>
    </svg>`;
  }
  ...
  function chip(cfg, rawHass) {
    ...
    const alertCol = '#f87171';
    return {
      icon: iconHtml(c.icon || 'mdi:water-alert'),
      label: c.label || 'Allagamento',
      value: ents.length ? `${active}/${ents.length}` : '—',
      color: active > 0 ? alertCol : '#4ade80',
    };
  }
  ...
  function render(cfg, rawHass) {
    ...
    const dryCount = ents.length - wetCount;
      ...
      if (wetCount === 0) {
        summary = `<div style="padding:10px 16px 6px">
          <div style="display:inline-flex;align-items:center;gap:6px;padding:6px 14px;border-radius:20px;background:rgba(74,222,128,.1);border:1px solid rgba(74,222,128,.22)">
            ...
            <span style="font-size:12px;font-weight:700;color:#4ade80">Tutto asciutto</span>
          </div>
        </div>`;
      } else {
        summary = `<div style="display:flex;gap:7px;padding:10px 16px 6px;flex-wrap:wrap">
          <div style="display:inline-flex;align-items:center;gap:5px;padding:5px 13px;border-radius:20px;background:rgba(248,113,113,.15);border:1px solid rgba(248,113,113,.4)">
            <span style="font-size:13px">🚨</span>
            <span style="font-size:12px;font-weight:800;color:#f87171">${wetCount} ALLAGATO${wetCount===1?'':'I'}!</span>
          </div>
          ${dryCount ? `<div style="display:inline-flex;align-items:center;gap:5px;padding:5px 13px;border-radius:20px;background:rgba(74,222,128,.09);border:1px solid rgba(74,222,128,.2)">
            <span style="font-size:12px;font-weight:700;color:#4ade80">✓ ${dryCount} asciutti</span>
          </div>` : ''}
        </div>`;
      }
    }
    ...
    const rows = ents.map((e, i) => {
      ...
      const stLbl  = wet ? '⚠️ Allagato' : 'Asciutto';
      const stCol  = wet ? '#f87171' : '#4ade80';
      const stBg   = wet ? 'rgba(248,113,113,.15)' : 'rgba(74,222,128,.12)';
      const stBdr  = wet ? 'rgba(248,113,113,.4)'  : 'rgba(74,222,128,.28)';
      ...
      return `<div style="border-bottom:1px solid rgba(255,255,255,.04);${wet?'background:rgba(248,113,113,.04)':''}">
        <div style="display:flex;align-items:center;gap:12px;padding:10px 16px">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${timeLabel ? `<div style="font-size:10px;color:${timeColor};margin-top:2px;font-weight:${wet?'700':'400'}">${eh(timeLabel)}</div>` : ''}
          </div>
    ...
    return `<div id="gpall-popup-body">
      ${summary}
  ...
  function configure(cfg, _el, onSave) {
    ...
    function _entityMatches(q) {
      ...
      return Object.keys(h.states)
        ...
        .map(id => {
          ...
          const icons = { binary_sensor:'💧', sensor:'📡' };
      ...
      return `<div style="width:100%;max-height:92vh;display:flex;flex-direction:column;background:#060d14;border:1px solid rgba(255,255,255,.12);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -16px 60px rgba(0,0,0,.9);color:#fff;${anim}">
        ...
        <div id="gpcfg-body" style="flex:1;overflow-y:auto;overflow-x:hidden;scrollbar-width:none;padding:14px 14px 4px">
          ...
          <div style="display:flex;gap:7px;margin-bottom:14px">
            <div style="flex:1"><div style="font-size:9px;color:#fff;margin-bottom:3px">Nome chip</div><input id="gpcfg-label" class="gpcinp" placeholder="Allagamento" value="${eh(c.label||'Allagamento')}"></div>
            <div style="flex:0 0 50px"><div style="font-size:9px;color:#fff;margin-bottom:3px">Colore</div><input type="color" id="gpcfg-color" value="${(c.color||'#38bdf8').match(/^#[0-9a-f]{6}$/i)?c.color:'#38bdf8'}" style="width:100%;height:36px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:none;cursor:pointer;padding:2px"></div>
          </div>
          ...
          <input id="gpcfg-add-entity" class="gpcinp" placeholder="🔍 Cerca binary_sensor.*…" autocomplete="off">
          <div style="margin-top:6px;padding:8px 10px;border-radius:8px;background:rgba(56,189,248,.06);border:1px solid rgba(56,189,248,.15);font-size:10px;color:#fff">
            💡 Aggiungi tutti i tuoi <strong style="color:#38bdf8">binary_sensor</strong> con <code>device_class: moisture</code>
          </div>
          <div style="height:16px"></div>
        </div>

        <div style="display:flex;gap:8px;padding:12px 14px;border-top:1px solid rgba(255,255,255,.06);flex-shrink:0">
          <button id="gpcfg-save" style="flex:1;padding:11px;border-radius:11px;border:none;background:#38bdf8;color:#060d14;font-weight:800;cursor:pointer;font-size:13px">💾 Salva</button>
    ...
    function attach() {
      ...
      ov.querySelector('#gpcfg-save').addEventListener('click', () => {
        const newCfg = {
          label:    (ov.querySelector('#gpcfg-label')?.value || 'Allagamento').trim(),
          color:    ov.querySelector('#gpcfg-color')?.value || '#38bdf8',
  ...
  const CARD = {
    ...
    desc: 'Chip sensori allagamento/umidità. Verde = tutto asciutto, rosso = allarme. Clic → dettaglio ogni sensore.',
    version: '1.0', isDistintivo: true,
    defaultCfg: { label: 'Allagamento', icon: 'mdi:water-alert', color: '#38bdf8', entities: [] },
  ...
  try { console.log('[FratechStore] Distintivo registrato: gruppo-allagamento v1.0'); } catch(e){}
})();
