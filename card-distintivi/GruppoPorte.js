/* frarik-version: 3.1 */
/**
 * GruppoPorte.js — Distintivo FratechStore v3.1
 * v3.1: popup config — riquadro unico (Chip/Entità/Aggiungi) con contorno
 *       bianco; righe entità a stile "glass" (badge icona rotondo, nome/
 *       entity-id distinti, automazione come pillola blu separata)
...
(function () {
  ...
  function configure(cfg, _el, onSave) {
    ...
    const presets = [
      { key: 'any_open',   label: 'Almeno una porta aperta' },
      { key: 'all_closed', label: 'Tutte le porte chiuse' },
    ...
    function _openAc(inp, matches, onPick) {
      ...
      _acDrop.style.cssText = `position:fixed;left:${rect.left}px;${pos};width:${rect.width}px;max-height:${MAXH}px;overflow-y:auto;z-index:100003;background:#1a1630;border:1px solid rgba(251,146,60,.3);border-radius:10px;box-shadow:0 8px 32px rgba(0,0,0,.88);scrollbar-width:thin;scrollbar-color:#fff transparent`;
      matches.forEach(m => {
        ...
        r.addEventListener('mouseover', () => { r.style.background='rgba(251,146,60,.08)'; });
    ...
    function renderForm() {
      ...
      return `<div style="width:100%;max-height:92vh;display:flex;flex-direction:column;background:#0a0816;border:1px solid rgba(255,255,255,.12);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -16px 60px rgba(0,0,0,.9);color:#fff;${anim}">
        ...
        <div style="display:flex;align-items:center;gap:10px;padding:14px 18px 12px;border-bottom:1px solid rgba(255,255,255,.06);flex-shrink:0">
          ...
          <div style="flex:1">
            <div style="font-size:14px;font-weight:800">Configura — Gruppo Porte</div>
        ...
        <div id="gpcfg-body" style="flex:1;overflow-y:auto;overflow-x:hidden;scrollbar-width:none;padding:14px 14px 4px">
          ...
          <div style="margin:0 0 14px;padding:14px;background:rgba(255,255,255,.05);border-radius:12px;border:1px solid #fff">
            ...
            <div style="display:flex;gap:7px">
              <div style="flex:1"><div style="font-size:9px;color:#fff;margin-bottom:3px">Nome chip</div><input id="gpcfg-label" class="gpcinp" placeholder="Porte" value="${eh(c.label||'Porte')}"></div>
  ...
  try { console.log('[FratechStore] Distintivo registrato: gruppo-porte v3.1'); } catch(e){}
})();
