/* frarik-version: 1.0 */
/**
 * GruppoFinestre.js — Distintivo FratechStore v1.0
...
(function () {
  ...
  function render(cfg, rawHass) {
    ...
    return `<div id="gf-popup-body">
      <div>${rows||'<div style="padding:32px 20px;text-align:center;color:rgba(255,255,255,.3);font-size:12px">Nessuna finestra configurata.<br><span style="font-size:10px;opacity:.6">Clicca ✏️ sulla chip per configurare.</span></div>'}</div>
    </div>`;
  }
  ...
  function configure(cfg, _el, onSave) {
    ...
    function _openAc(inp, matches, onPick) {
      ...
      _acDrop.style.cssText = `position:fixed;left:${rect.left}px;${pos};width:${rect.width}px;max-height:${MAXH}px;overflow-y:auto;z-index:100003;background:#1a1630;border:1px solid rgba(52,211,153,.3);border-radius:10px;box-shadow:0 8px 32px rgba(0,0,0,.88);scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.1) transparent`;
      matches.forEach(m => {
        ...
        const stColor = m.on ? '#34d399' : 'rgba(255,255,255,.3)';
    ...
    function renderForm() {
      ...
      return `<div style="width:100%;max-height:92vh;display:flex;flex-direction:column;background:#0f0d1a;border:1px solid rgba(52,211,153,.22);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -16px 60px rgba(0,0,0,.9);color:#fff;${anim}">
        <style>@keyframes gfCfgUp{from{transform:translateY(100%)}to{transform:translateY(0)}} .gfcinp{width:100%;box-sizing:border-box;padding:8px 10px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:#fff;font-size:12px;outline:none;font-family:inherit;transition:border-color .15s} .gfcinp:focus{border-color:rgba(52,211,153,.5);background:rgba(52,211,153,.04)} .gfcinp::placeholder{color:rgba(255,255,255,.3)} #gfcfg-body::-webkit-scrollbar{display:none}</style>
        ...
        <div id="gfcfg-body" style="flex:1;overflow-y:auto;overflow-x:hidden;scrollbar-width:none;padding:14px 14px 4px">
          ...
          <div style="display:flex;gap:7px;margin-bottom:14px">
            <div style="flex:1"><div style="font-size:9px;color:rgba(255,255,255,.4);margin-bottom:3px">Nome chip</div><input id="gfcfg-label" class="gfcinp" placeholder="Finestre" value="${eh(c.label||'Finestre')}"></div>
            <div style="flex:0 0 56px"><div style="font-size:9px;color:rgba(255,255,255,.4);margin-bottom:3px">Icona</div><button id="gfcfg-icon-btn" style="width:100%;height:36px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);cursor:pointer;display:flex;align-items:center;justify-content:center;outline:none">${iconHtml(c.icon||'🪟',22)}</button><input type="hidden" id="gfcfg-icon" value="${eh(c.icon||'🪟')}"></div>
            <div style="flex:0 0 50px"><div style="font-size:9px;color:rgba(255,255,255,.4);margin-bottom:3px">Colore</div><input type="color" id="gfcfg-color" value="${(c.color||'#34d399').match(/^#[0-9a-f]{6}$/i)?c.color:'#34d399'}" style="width:100%;height:36px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:none;cursor:pointer;padding:2px"></div>
          </div>
          ...
          <div style="font-size:9px;color:rgba(255,255,255,.25);margin-top:5px">Mostra tutte le entità — binary_sensor.* compaiono per prime</div>
          <div style="height:16px"></div>
        </div>

        <div style="display:flex;gap:8px;padding:12px 14px;border-top:1px solid rgba(255,255,255,.06);flex-shrink:0">
          <button id="gfcfg-save" style="flex:1;padding:11px;border-radius:11px;border:none;background:#34d399;color:#0a0816;font-weight:800;cursor:pointer;font-size:13px">💾 Salva</button>
  ...
  try { console.log('[FratechStore] Distintivo registrato: gruppo-finestre v1.0'); } catch(e){}
})();
