...
(function () {
  ...
  /* module-level UI state (survives re-render, resets on page reload) */
  var _timerOpen = {};
  ...
  function render(card) {
    ...
    var sourceList = attrs.source_list || [];
    ...
    var bTmr = 'width:32px;height:32px;border-radius:9px;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;user-select:none;-webkit-tap-highlight-color:transparent;font-size:13px;'
      + (timerOpen ? 'border:1px solid ' + col + ';background:rgba(' + rgb + ',.22);color:' + col + ';box-shadow:0 0 8px rgba(' + rgb + ',.4)' : 'border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.05);color:rgba(255,255,255,.6)');
    ...
    if (timerOpen) {
      ...
      timerHtml = '<div style="display:flex;gap:5px;padding:0 14px 8px;flex-shrink:0">'
    ...
    var presetsHtml = '<div style="display:flex;gap:5px;padding:0 14px 6px;flex-shrink:0">'
      + [25,50,75,100].map(function(p) {
          ...
          return '<div style="flex:1;padding:5px 0;text-align:center;border-radius:8px;cursor:pointer;font-size:10px;font-weight:700;user-select:none;-webkit-tap-highlight-color:transparent;'
            + (active
              ? 'background:rgba(' + rgb + ',.22);border:1px solid ' + col + ';color:' + col
    ...
    return css
      ...
      + ctrlHtml
      + (timerOpen ? '<div class="fc-sep"></div>' + timerHtml : '')
      + '<div class="fc-sep"></div>'
      + presetsHtml
      + volHtml
      + '<div class="fc-sep"></div>'
  ...
  function openSourcePicker(card, el) {
    ...
    var attrs = (h && h.states && h.states[eid] && h.states[eid].attributes) || {};
    var sources = attrs.source_list || [];
    var curSrc  = attrs.source || '';
    var col = c.color, rgb = hexRgb(col);

    var listHtml = sources.length
      ? sources.map(function(src) {
          var active = src === curSrc;
          return '<div data-src="'+_esc(src)+'" style="padding:12px 16px;cursor:pointer;display:flex;align-items:center;gap:10px;border-bottom:1px solid rgba(255,255,255,.05);'
            +(active?'background:rgba('+rgb+',.1);':'')
            +'">'
    ...
    ov.querySelectorAll('[data-src]').forEach(function(row) {
      ...
      row.addEventListener('click', function() {
        callSvc('media_player','select_source',{entity_id:eid, source:row.getAttribute('data-src')});
        ov._close();
      });
    });
  }
  ...
  function openCfg(card, el) {
    ...
    var formHtml='<div style="margin-bottom:10px"><label style="'+stLbl+'">Nome card</label>'
      ...
      +fldAC('axc-player','Media Player',c.pk_player,'media_player.sfera_piano_terra',mpIds)
      +'<div style="'+stSec+'">TTS</div>'
      +fldAC('axc-notify','Servizio notify Alexa',c.pk_notify,'es. alexa_media',['alexa_media'])
    ...
    var ov=mkOv(popShell('🔊','244,114,182','Configura Alexa',card.id||'','axc-cfg-close',
      '<div style="padding:13px 15px">'+formHtml+'</div>'),'axc-cfg-close');
  ...
  function update(card, hass, el) {
    ...
    var sig=[CARD.version,st,at.media_title,at.media_artist,at.entity_picture,
             ...
             at.source, !!_timerOpen[card.id||'x']].join('|');
  ...
  function mount(card, hass, el) {
    ...
    function sendTts(text) {
      ...
      callSvc('notify', notifSvc(), { target:[eid()], message:text, data:{type:'tts'} });
    }
    ...
    el._axHandler = function(e) {
      ...
      if (a==='rename') {
        ...
        function commit(){
          ...
          el._axTtsFocus=false; el._axSig=''; el._axBound=null;
          el.innerHTML=render(card); mount(card,null,el);
        }
        inp.addEventListener('blur',commit,{once:true});
        inp.addEventListener('keydown',function(e){
          if(e.key==='Enter'){inp.blur();}
          if(e.key==='Escape'){inp.removeEventListener('blur',commit);t.textContent=cur;el._axTtsFocus=false;}
        });
        return;
      }
      ...
      if (a==='vol-preset') {
        var pct=parseInt(t.dataset.vol)||50;
        callSvc('media_player','volume_set',{entity_id:eid(),volume_level:pct/100});
        applyVolUI(el, pct); return;
      }
      if (a==='timer-toggle') {
        _timerOpen[cid]=!_timerOpen[cid];
        el._axSig=''; el._axBound=null; el.innerHTML=render(card); mount(card,null,el); return;
      }
      if (a==='timer-set') {
        var min=parseInt(t.dataset.min)||5;
        var msg=min>=60?'imposta un timer di '+(min/60)+' '+(min/60===1?'ora':'ore'):'imposta un timer di '+min+' minuti';
        sendTts(msg); return;
      }
      if (a==='tts-send') {
        var inp2=el.querySelector('[data-axa="tts-inp"]');
        if(inp2){sendTts(inp2.value);inp2.value='';inp2.blur();} return;
      }
    };
    el.addEventListener('click', el._axHandler);

    /* TTS focus guard + Enter */
    ...
    el._axVolTS=function(e){
      ...
      function sv(ev){var t=(ev.touches||ev.changedTouches)[0];if(!t) return;applyVolUI(el,Math.round(r(t)*100));clearTimeout(el._axVolTimer);el._axVolTimer=setTimeout(function(){callSvc('media_player','volume_set',{entity_id:eid(),volume_level:r(t)});},80);}
      sv(e);
      function onMove(ev){sv(ev);}
      function onEnd(ev){el.removeEventListener('touchmove',onMove);el.removeEventListener('touchend',onEnd);clearTimeout(el._axVolTimer);var t=(ev.changedTouches||[])[0];if(t)callSvc('media_player','volume_set',{entity_id:eid(),volume_level:Math.round(r(t)*100)/100});setTimeout(function(){el._axVolDragging=false;},300);}
  ...
  /* ── REGISTRATION ── */
  var CARD = {
    id:'alexa-card', name:'Alexa Media', icon:'🔊', version:'1.7',
    desc:'Controllo Alexa: album art, equalizzatore, sorgente, preset volume, timer, TTS inline.',
  ...
  try{console.log('[FratechStore] Card registrata: alexa-card v1.7');}catch(e){}
})();
