...
(function () {
  ...
  function openImpostazioniHAPopup(c) {
    ...
    dTime('input_datetime.frarik_lavatrice_notifiche_inizio', '⏰ Orario inizio notifiche');
    dTime('input_datetime.frarik_lavatrice_notifiche_fine',   '⏰ Orario fine notifiche');
    ...
    dTime('input_datetime.frarik_lavatrice_off', 'Orario spegnimento');
    ...
    const ov = mkOv(popShell('⚙', '100,116,139', 'Impostazioni', c.name || 'Lavatrice', closeId, swCss + rows.join('') + saveBtn + resetBtn), closeId);
...
`;
  ...
  /* ── CARD ── */
  const CARD = {
    id: 'lavatrice', name: 'Lavatrice', icon: '🫧', version: '1.7',
