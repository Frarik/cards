...
/* v2.4: popup Energia/Cicli/Impostazioni allineati allo standard delle altre
   ...
   più visibile (.08→.16, stesso valore delle altre card); pulsante "Salva
   impostazioni" ora blu pieno invece che translucido; etichette dei campi
   nel popup Impostazioni ora maiuscolo/grassetto; rimaste scritte sbiadite
   portate a bianco pieno. Aggiunta sezione "Aspetto" nel popup Impostazioni
   con zoom/larghezza card e anteprima live (mancava del tutto). */
(function () {
  'use strict';
  ...
  function render(card) {
    ...
    return css
      ...
      + '<div class="fc-hdr-tit">' + (S(h, 'input_text.frarik_lavatrice_nome') || c.name || 'Lavatrice') + '</div>'
  ...
  function openEnergiaPopup(c) {
    ...
    const content = '<div style="background:rgba(56,189,248,.1);border-radius:12px;padding:12px 14px;text-align:center;margin-bottom:12px">'
      ...
      + row('Mese precedente', fmtEur(Attr(h, ton, 'costo_mese_precedente_lavatrice')), '#fff')
      + row('Questo anno', fmtEur(Attr(h, ton, 'costo_anno_lavatrice')), '#7dd3fc')
      + row('Anno precedente', fmtEur(Attr(h, ton, 'costo_anno_precedente_lavatrice')), '#fff');
  ...
  function openCfg(card, el) {
    ...
    const ov = mkOv(popShell('🫧', 'Configura Lavatrice', 'fc-cfg-close', formHtml), 'fc-cfg-close');
  ...
  function openImpostazioniHAPopup(card, c) {
    ...
    dToggle('input_boolean.frarik_lavatrice_notify_push',   '📱 Push');
    dToggle('input_boolean.frarik_lavatrice_notify_google', '🔊 Google');
    dToggle('input_boolean.frarik_lavatrice_notify_alexa',  '🗣 Alexa');
    dTime('input_datetime.frarik_lavatrice_orario_inizio_notifiche', '⏰ Orario inizio notifiche');
    dTime('input_datetime.frarik_lavatrice_orario_fine_notifiche',   '⏰ Orario fine notifiche');
    ...
    dToggle('automation.frarik_lavatrice_off_automatico', 'Auto OFF abilitato');
    dTime('input_datetime.frarik_lavatrice_off_automatico', 'Orario spegnimento');

    dSec('📝 Personalizzazione');
    dText('input_text.frarik_lavatrice_nome',      'Nome elettrodomestico');
    dText('input_text.frarik_lavatrice_messaggio', 'Messaggio notifica');
    dNum('input_number.costo_energia',  'Costo energia', '€/kWh', 0, 2, 0.001);
    dInfo('Ultimo reset contatori', ss('input_text.frarik_lavatrice_data_reset'));
...
homeassistant:
  customize:
    package.node_anchors:
      customize: &customize
        package: 'Frarik — Centro Controllo Lavatrice 2.0 — Frarik'
      setting:
...
####################################################
        ...
        Sensore Potenza Lavatrice: &sensore_potenza   "{{ states('IL_TUO_SENSORE_POTENZA') | float(0) }}"
      ...
      - name: frarik_lavatrice_time_on
        ...
        attributes:
          ...
          Mese Precedente: >
            ...
            {% endif %}
          Anno Precedente: >
            {% set hours = state_attr('sensor.frarik_lavatrice_tempo_anno', 'last_period') | float(0) %}
            {% set minutes = ((hours % 1) * 60) | int(0) %}
            {% set hours = (hours - (hours % 1)) | int(0) %}
            {% set day = ((hours | int(0) / 24)) | int(0) %}
            {% if day | int(0) > 0 %}
          ...
          consumo_ciclo_lavatrice: >-
            {{ (states('sensor.frarik_lavatrice_kwh') | float(0) - states('sensor.frarik_lavatrice_inizio_ciclo') | float(0)) | round(2) }} kWh
          costo_ciclo_lavatrice: >-
            {{ ((states('sensor.frarik_lavatrice_kwh') | float(0) - states('sensor.frarik_lavatrice_inizio_ciclo') | float(0)) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_oggi_lavatrice: >-
            {{ (states('sensor.frarik_lavatrice_energy_oggi') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_mese_lavatrice: >-
            {{ (states('sensor.frarik_lavatrice_energy_mese') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_anno_lavatrice: >-
            {{ (states('sensor.frarik_lavatrice_energy_anno') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_ieri_lavatrice: >-
            {{ (state_attr('sensor.frarik_lavatrice_energy_oggi', 'last_period') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_mese_precedente_lavatrice: >-
            {{ (state_attr('sensor.frarik_lavatrice_energy_mese', 'last_period') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_anno_precedente_lavatrice: >-
            {{ (state_attr('sensor.frarik_lavatrice_energy_anno', 'last_period') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
...
input_datetime:
  ...
  frarik_lavatrice_orario_inizio_notifiche:
    name: Orario Inizio Notifiche Lavatrice
    has_date: false
    has_time: true

  frarik_lavatrice_orario_fine_notifiche:
    name: Orario Fine Notifiche Lavatrice
    has_date: false
    has_time: true

  frarik_lavatrice_off_automatico:
    name: Lavatrice Off Automatico
    has_date: false
    has_time: true
...
- alias: frarik_lavatrice_automazione
  id: frarik_lavatrice_automazione
  max_exceeded: silent
  trigger:
    ...
    - conditions:
      ...
      - service: input_text.set_value
        target:
          entity_id: >
            ...
            {{ {'Monday':'input_text.frarik_lavatrice_cicli_lunedi','Tuesday':'input_text.frarik_lavatrice_cicli_martedi','Wednesday':'input_text.frarik_lavatrice_cicli_mercoledi','Thursday':'input_text.frarik_lavatrice_cicli_giovedi','Friday':'input_text.frarik_lavatrice_cicli_venerdi','Saturday':'input_text.frarik_lavatrice_cicli_sabato','Sunday':'input_text.frarik_lavatrice_cicli_domenica'}[g] }}
        data:
      ...
      - service: input_text.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_text.frarik_lavatrice_tempo_lunedi','Tuesday':'input_text.frarik_lavatrice_tempo_martedi','Wednesday':'input_text.frarik_lavatrice_tempo_mercoledi','Thursday':'input_text.frarik_lavatrice_tempo_giovedi','Friday':'input_text.frarik_lavatrice_tempo_venerdi','Saturday':'input_text.frarik_lavatrice_tempo_sabato','Sunday':'input_text.frarik_lavatrice_tempo_domenica'}[g] }}
        data:
      ...
      - service: input_number.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_number.frarik_lavatrice_consumo_lunedi','Tuesday':'input_number.frarik_lavatrice_consumo_martedi','Wednesday':'input_number.frarik_lavatrice_consumo_mercoledi','Thursday':'input_number.frarik_lavatrice_consumo_giovedi','Friday':'input_number.frarik_lavatrice_consumo_venerdi','Saturday':'input_number.frarik_lavatrice_consumo_sabato','Sunday':'input_number.frarik_lavatrice_consumo_domenica'}[g] }}
        data:
      ...
      - service: input_number.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_number.frarik_lavatrice_costo_lunedi','Tuesday':'input_number.frarik_lavatrice_costo_martedi','Wednesday':'input_number.frarik_lavatrice_costo_mercoledi','Thursday':'input_number.frarik_lavatrice_costo_giovedi','Friday':'input_number.frarik_lavatrice_costo_venerdi','Saturday':'input_number.frarik_lavatrice_costo_sabato','Sunday':'input_number.frarik_lavatrice_costo_domenica'}[g] }}
        data:
    ...
    - choose:
      - conditions:
        - condition: trigger
          id: fine_ciclo
        - condition: time
          after: 'input_datetime.frarik_lavatrice_orario_inizio_notifiche'
          before: 'input_datetime.frarik_lavatrice_orario_fine_notifiche'
        - condition: state
        ...
        - service: tts.google_translate_say
          ...
          data:
            entity_id: *google
            message: >-
              {%- set dur=states('input_text.frarik_lavatrice_ultimo_ciclo')|trim -%}
              ...
              {{- states('input_text.frarik_lavatrice_messaggio') }} in {{ hv+sep+mv -}}
      ...
      - conditions:
        ...
        - repeat:
            ...
            - service: "notify.{{ repeat.item.service }}"
              continue_on_error: true
              data:
                title: "{{ states('input_text.frarik_lavatrice_nome') }}"
                message: >-
                  🫧 {{ states('input_text.frarik_lavatrice_messaggio') }}
...
- alias: frarik_lavatrice_off_automatico
  ...
  trigger:
    - platform: time
      at: 'input_datetime.frarik_lavatrice_off_automatico'
      id: auto_off
  condition: []
  action:
    - choose:
...
`;
  ...
  function _buildPkg(potenza, sw, push, google, alexa, _tpl) {
    ...
    var yaml = (_tpl || _LAVATRICE_PKG_YAML)
  ...
  function _openWizard(hass, onDone, _tpl, opts) {
    ...
    function renderWiz() {
      sr.innerHTML = '<style>'
        ...
        + '<div><div class="wd-tit">' + (isUpdate ? 'Aggiorna PKG Lavatrice' : 'Installa PKG Lavatrice') + '</div><div class="wd-sub">frarik_lavatrice.yaml → config/packages/</div></div>'
  ...
  /* ── CARD ── */
  const CARD = {
    id: 'lavatrice', name: 'Lavatrice', icon: '🫧', version: '2.4',
