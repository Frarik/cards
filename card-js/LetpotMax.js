...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:5px;padding:6px 10px 10px;flex:1;align-content:end">
  ...
  function openSettings(card, el){
    ...
    sh.innerHTML=`<style>#lps-body *{text-transform:uppercase!important;font-weight:800!important}</style><div style="display:flex;align-items:center;gap:12px;padding:18px 20px 14px;border-bottom:1px solid rgba(255,255,255,.06);flex-shrink:0">
  ...
  const CARD={id:ID,name:'LetPot Max',icon:'🌿',desc:'Sistema idroponico LPH-MAX 21 pod — luce che cambia tonalità in base alla modalità, timer pompa reale 30min, impostazioni in un pannello dedicato',version:'4.11',colSpan:2,rowSpan:3,frarik_no_edit:true,render,mount,update,configure,preview};
  ...
  try{console.log('[FratechStore] letpot-max v4.11');}catch(e){}
})();
