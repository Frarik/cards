...
(function () {
  ...
  function keyOf(c) { return 'frarik_tostapanecard_' + (c.id || 'x'); }
  ...
  function pkDefaults() {
    return {
      pk_power:      'sensor.frarik_tostapane_potenza_w',
      pk_running:    'binary_sensor.frarik_tostapane_motore',
      pk_switch:     'switch.presa_tostapane',
      pk_kwh_oggi:   'sensor.frarik_tostapane_energy_oggi',
      pk_kwh_mese:   'sensor.frarik_tostapane_energy_mese',
      pk_kwh_anno:   'sensor.frarik_tostapane_energy_anno',
      pk_cicli_oggi: 'sensor.frarik_tostapane_cicli_oggi',
      pk_cicli_mese: 'sensor.frarik_tostapane_cicli_mese',
      pk_cicli_anno: 'sensor.frarik_tostapane_cicli_anno',
      pk_cicli_tot:  'counter.frarik_tostapane_cicli_totale',
      pk_time_on:    'sensor.frarik_tostapane_time_on',
      pk_soglia:     'input_number.frarik_tostapane_soglia_w',
      pk_versione:   'sensor.frarik_tostapane_versione',
    };
  }
  ...
  function cfgFor(card) {
    ...
    r.name = c.name || 'Tostapane';
    return r;
  }

  /* ── TOSTAPANE SVG ── */
  function _tostapaneSVG(running) {
    var c    = running ? '#f97316' : '#64748b';
    var cf   = running ? 'rgba(249,115,22,.1)' : 'rgba(100,116,139,.06)';
    var glow = running ? ';filter:drop-shadow(0 0 12px rgba(249,115,22,.4))' : '';
    var css  = running ? '@keyframes tpheat{0%,100%{opacity:.3}50%{opacity:.9}}@keyframes tpbread{0%{transform:translateY(0)}50%{transform:translateY(-4px)}100%{transform:translateY(0)}}' : '';
    var hAnim = running ? 'style="animation:tpheat 1.2s ease-in-out infinite"' : '';
    var h2Anim = running ? 'style="animation:tpheat 1.2s ease-in-out infinite .4s"' : '';
    var h3Anim = running ? 'style="animation:tpheat 1.2s ease-in-out infinite .8s"' : '';
    var breadAnim = running ? 'style="animation:tpbread 2s ease-in-out infinite"' : '';
    return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 106" style="display:block;width:100%;height:100%' + glow + '">'
      ...
      // Body
      + '<rect x="8" y="40" width="64" height="38" rx="10" fill="#080f1a" stroke="' + c + '" stroke-width="1.5"/>'
      + '<rect x="10" y="42" width="60" height="34" rx="8" fill="' + cf + '"/>'
      // Slot sinistro
      + '<rect x="16" y="35" width="18" height="28" rx="5" fill="#040a12" stroke="' + c + '" stroke-width="1.2"/>'
      // Slot destro
      + '<rect x="46" y="35" width="18" height="28" rx="5" fill="#040a12" stroke="' + c + '" stroke-width="1.2"/>'
      // Pane sinistro
      + '<g ' + breadAnim + '>'
      + '<rect x="18" y="20" width="14" height="18" rx="3" fill="' + (running ? '#d97706' : '#78350f') + '" stroke="' + c + '" stroke-width=".8" opacity="' + (running ? '.95' : '.5') + '"/>'
      + '<rect x="19" y="21" width="12" height="3" rx="1" fill="rgba(255,255,255,.1)"/>'
      + '</g>'
      // Pane destro
      + '<g ' + breadAnim + '>'
      + '<rect x="48" y="20" width="14" height="18" rx="3" fill="' + (running ? '#d97706' : '#78350f') + '" stroke="' + c + '" stroke-width=".8" opacity="' + (running ? '.95' : '.5') + '"/>'
      + '<rect x="49" y="21" width="12" height="3" rx="1" fill="rgba(255,255,255,.1)"/>'
      + '</g>'
      // Resistenze visibili
      + '<line x1="16" y1="55" x2="34" y2="55" stroke="' + (running ? '#f97316' : '#1a3050') + '" stroke-width="1.5" ' + hAnim + '/>'
      + '<line x1="16" y1="60" x2="34" y2="60" stroke="' + (running ? '#fb923c' : '#1a3050') + '" stroke-width="1.5" ' + h2Anim + '/>'
      + '<line x1="16" y1="65" x2="34" y2="65" stroke="' + (running ? '#f97316' : '#1a3050') + '" stroke-width="1.5" ' + h3Anim + '/>'
      + '<line x1="46" y1="55" x2="64" y2="55" stroke="' + (running ? '#f97316' : '#1a3050') + '" stroke-width="1.5" ' + hAnim + '/>'
      + '<line x1="46" y1="60" x2="64" y2="60" stroke="' + (running ? '#fb923c' : '#1a3050') + '" stroke-width="1.5" ' + h2Anim + '/>'
      + '<line x1="46" y1="65" x2="64" y2="65" stroke="' + (running ? '#f97316' : '#1a3050') + '" stroke-width="1.5" ' + h3Anim + '/>'
      // Leva / maniglia
      + '<rect x="36" y="50" width="8" height="18" rx="4" fill="#060c18" stroke="' + c + '" stroke-width="1"/>'
      + '<rect x="37" y="60" width="6" height="6" rx="2" fill="' + c + '" opacity=".5"/>'
      // Gambe
      + '<rect x="14" y="77" width="6" height="8" rx="3" fill="' + c + '" opacity=".4"/>'
      + '<rect x="60" y="77" width="6" height="8" rx="3" fill="' + c + '" opacity=".4"/>'
      // LED
      + '<circle cx="68" cy="48" r="3" fill="' + (running ? '#f97316' : '#0a1a2e') + '"/>'
      + '</svg>';
  }
  ...
  function render(card) {
    ...
    var _sgCached = parseFloat(localStorage.getItem('_fsg_' + c.pk_soglia)); const sogliaN = !isNaN(_sgCached) ? _sgCached : (num(S(h, c.pk_soglia)) || 500);
    const running = isOn(h, c.pk_running) || (pwV != null && pwV >= sogliaN);
    const ton     = c.pk_time_on;

    const terminato  = Attr(h, ton, 'terminato')                || '—';
    const tempoC     = Attr(h, ton, 'tempo_ciclo_tostapane')    || '—';
    const consumoC   = Attr(h, ton, 'consumo_ciclo_tostapane')  || '—';
    const costoC     = Attr(h, ton, 'costo_ciclo_tostapane');
    const kwOggi     = S(h, c.pk_kwh_oggi);
    const cicOggi    = S(h, c.pk_cicli_oggi);
    const timeOggi   = Attr(h, ton, 'Oggi')                     || '—';
    const costoOggi  = Attr(h, ton, 'costo_oggi_tostapane');

    const pw     = pwV || 0;
    const col    = running ? '#f97316' : '#64748b';
    const statusLabel = running ? 'TOSTATURA ON' : 'STANDBY';
    const barMax = 1500; const barPct = Math.min(100, (pw / barMax) * 100);
    const barCol = pw < 20 ? '#64748b' : pw <= 400 ? '#22c55e' : pw <= 800 ? '#eab308' : pw <= 1200 ? '#f97316' : '#ef4444';
    ...
    const css = '<style>'
      ...
      + '#' + rid + ' .fc-card::before{content:"";position:absolute;top:0;left:0;right:0;height:200px;background:radial-gradient(ellipse at 20% 0%,rgba(249,115,22,.08) 0%,transparent 65%);pointer-events:none}'
      ...
      + '#' + rid + ' .fc-hdr-iw{width:26px;height:26px;border-radius:7px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:14px;background:rgba(249,115,22,.1);border:1px solid rgba(249,115,22,.2)}'
      ...
      + '#' + rid + ' .fc-dot{width:6px;height:6px;border-radius:50%;flex-shrink:0;background:#f97316' + (running ? ';box-shadow:0 0 5px #f97316;animation:fcPulse 1.5s ease-in-out infinite' : '') + '}'
      ...
      + '#' + rid + ' .fc-stdot{width:8px;height:8px;border-radius:50%;background:#f97316;flex-shrink:0' + (running ? ';box-shadow:0 0 7px #f97316;animation:fcPulse 1.5s ease-in-out infinite' : '') + '}'
      ...
      + '#' + rid + ' .fc-sb-n{font-size:12px;font-weight:900;color:#f97316;height:18px;display:flex;align-items:center;justify-content:center}'
      ...
      + '#' + rid + ' .fc-btn:hover{background:rgba(249,115,22,.12);border-color:rgba(249,115,22,.3);color:#f97316}'
    ...
    const heroHtml = '<div class="fc-hero">'
      + '<div class="fc-hero-img" data-sya="popup-cicli">' + _tostapaneSVG(running) + '</div>'
      + '<div class="fc-hero-r">'
      + '<div class="fc-st">' + (running ? 'In tostatura' : 'Standby') + '<div class="fc-stdot"></div></div>'
    ...
    return css
      ...
      + '<div class="fc-hdr-tit">' + (S(h, 'input_text.frarik_tostapane_nome') || c.name || 'Tostapane') + '</div>'
      + '<div class="fc-hdr-pill" style="background:' + (running ? 'rgba(249,115,22,.1)' : 'rgba(249,115,22,.05)') + ';border:1px solid rgba(249,115,22,' + (running ? '.28' : '.15') + ');color:#f97316">'
  ...
  function openEnergiaPopup(c) {
    const h = H();
    function row(lbl, val, col) { return '<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.05)"><span style="font-size:12px;color:#fff">' + lbl + '</span><span style="font-size:13px;font-weight:800;color:' + (col || '#f97316') + '">' + val + '</span></div>'; }
    const ton = c.pk_time_on;
    const pwV = num(S(h, c.pk_power));
    const content = '<div style="background:rgba(249,115,22,.1);border-radius:12px;padding:12px 14px;text-align:center;margin-bottom:12px">'
      ...
      + '<div style="font-size:28px;font-weight:900;color:#f97316">' + (pwV == null ? '—' : pwV.toFixed(0) + ' W') + '</div>'
      + '</div>'
      ...
      + row('Oggi',             fmtEur(Attr(h, ton, 'costo_oggi_tostapane')),        '#fb923c')
      + row('Ieri',             fmtEur(Attr(h, ton, 'costo_ieri_tostapane')),        '#fff')
      + row('Questo mese',      fmtEur(Attr(h, ton, 'costo_mese_tostapane')),        '#fb923c')
      + row('Mese precedente',  fmtEur(Attr(h, ton, 'costo_mese_precedente_tostapane')), '#fff')
      + row('Questo anno',      fmtEur(Attr(h, ton, 'costo_anno_tostapane')),        '#fb923c')
      + row('Anno precedente',  fmtEur(Attr(h, ton, 'costo_anno_precedente_tostapane')), '#fff');
    mkOv(popShell('⚡', '249,115,22', 'Energia & Costi', 'Tostapane', 'fc-en-close', content), 'fc-en-close');
  }
  ...
  function openCicliPopup(c) {
    ...
    DAYS.forEach(function (d, i) {
      ...
      const cicli   = isToday ? cleanVal(S(h, c.pk_cicli_oggi))     : cleanVal(S(h, 'input_text.frarik_tostapane_cicli_' + d));
      const tempo   = isToday ? cleanVal(Attr(h, ton, 'Oggi'))       : cleanVal(S(h, 'input_text.frarik_tostapane_tempo_' + d));
      const kwRaw   = isToday ? S(h, c.pk_kwh_oggi) : S(h, 'input_number.frarik_tostapane_consumo_' + d);
      const costRaw = isToday ? Attr(h, ton, 'costo_oggi_tostapane') : S(h, 'input_number.frarik_tostapane_costo_' + d);
      const kwFmt   = num(kwRaw)   != null ? num(kwRaw).toFixed(2)   + 'k' : '—';
      const costFmt = num(costRaw) != null ? num(costRaw).toFixed(2) + '€' : '—';
      weekHtml += '<div style="display:flex;flex-direction:column;align-items:center;gap:2px;background:' + (isToday ? 'rgba(249,115,22,.12)' : 'rgba(249,115,22,.05)') + ';border:1px solid ' + (isToday ? 'rgba(249,115,22,.4)' : 'rgba(249,115,22,.1)') + ';border-radius:8px;padding:6px 2px' + (isToday ? ';box-shadow:0 0 8px rgba(249,115,22,.15)' : '') + '">'
        + '<div style="font-size:8px;font-weight:' + (isToday ? '900' : '700') + ';color:' + (isToday ? '#f97316' : '#fff') + '">' + DAY_LABELS[i] + '</div>'
        + '<div style="font-size:11px;font-weight:900;color:#f97316">' + cicli + '</div>'
    ...
    const content = weekHtml
      + '<div style="font-size:10px;font-weight:700;color:#fff;text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Cicli tostatura</div>'
      + row('Oggi',          S(h, c.pk_cicli_oggi) || '—', '#f97316')
      + row('Questo mese',   S(h, c.pk_cicli_mese) || '—', '#f97316')
      + row('Questo anno',   S(h, c.pk_cicli_anno) || '—', '#f97316')
      + row('Totale storico', S(h, c.pk_cicli_tot) || '—', '#fb923c')
      + '<div style="font-size:10px;font-weight:700;color:#fff;text-transform:uppercase;letter-spacing:.06em;margin:12px 0 4px">Tempo tostatura</div>'
      + row('Oggi',            Attr(h, ton, 'Oggi')           || '—', '#fb923c')
      + row('Ieri',            Attr(h, ton, 'Ieri')           || '—', '#fff')
      + row('Questo mese',     Attr(h, ton, 'Mese')           || '—', '#fb923c')
      + row('Mese precedente', Attr(h, ton, 'Mese Precedente')|| '—', '#fff')
      + row('Questo anno',     Attr(h, ton, 'Anno')           || '—', '#fb923c')
      ...
      + row('Ultimo ciclo',  fmtEur(Attr(h, ton, 'costo_ciclo_tostapane')), '#fb923c')
      + row('Oggi',          fmtEur(Attr(h, ton, 'costo_oggi_tostapane')),  '#fb923c')
      + row('Questo mese',   fmtEur(Attr(h, ton, 'costo_mese_tostapane')),  '#fff')
      + row('Questo anno',   fmtEur(Attr(h, ton, 'costo_anno_tostapane')),  '#fff');
    mkOv(popShell('🍞', '249,115,22', 'Cicli & Statistiche', 'Resistenza tostapane', 'fc-ci-close', content), 'fc-ci-close');
  }
  ...
  function openCfg(card, el) {
    ...
    const formHtml = '<div style="margin-bottom:10px"><label style="' + stLbl + '">Nome card</label><input id="fc-name" type="text" value="' + (cf.name || '').replace(/"/g, '&quot;') + '" placeholder="es. Tostapane cucina" style="' + stInp.replace('monospace', 'system-ui') + '"></div>'
      + '<div style="' + stSec + '">Sensori base</div>'
      + field('fc-power',   'Potenza istantanea (W)', cf.pk_power,   'sensor.frarik_tostapane_potenza_w')
      + field('fc-running', 'Resistenza on/off',      cf.pk_running, 'binary_sensor.frarik_tostapane_motore')
      + field('fc-switch',  'Switch presa',            cf.pk_switch,  'switch.presa_tostapane')
      + '<div style="' + stSec + '">PKG — Energia (kWh)</div>'
      + field('fc-kwh-oggi', 'kWh oggi', cf.pk_kwh_oggi, 'sensor.frarik_tostapane_energy_oggi')
      + field('fc-kwh-mese', 'kWh mese', cf.pk_kwh_mese, 'sensor.frarik_tostapane_energy_mese')
      + field('fc-kwh-anno', 'kWh anno', cf.pk_kwh_anno, 'sensor.frarik_tostapane_energy_anno')
      + '<div style="' + stSec + '">PKG — Cicli</div>'
      + field('fc-cic-oggi', 'Cicli oggi',   cf.pk_cicli_oggi, 'sensor.frarik_tostapane_cicli_oggi')
      + field('fc-cic-mese', 'Cicli mese',   cf.pk_cicli_mese, 'sensor.frarik_tostapane_cicli_mese')
      + field('fc-cic-anno', 'Cicli anno',   cf.pk_cicli_anno, 'sensor.frarik_tostapane_cicli_anno')
      + field('fc-cic-tot',  'Cicli totale', cf.pk_cicli_tot,  'counter.frarik_tostapane_cicli_totale')
      + '<div style="' + stSec + '">PKG — Statistiche</div>'
      + field('fc-time-on', 'Sensore time_on',   cf.pk_time_on, 'sensor.frarik_tostapane_time_on')
      + field('fc-soglia',  'Soglia lavoro (W)', cf.pk_soglia,  'input_number.frarik_tostapane_soglia_w')
    ...
    const ov = mkOv(popShell('🍞', '249,115,22', 'Configura Tostapane', card.id || '', 'fc-cfg-close', formHtml), 'fc-cfg-close');
  ...
  function openImpostazioniHAPopup(c) {
    ...
    function dSec(lbl) { rows.push('<div style="font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:#f97316;padding:12px 0 6px;border-bottom:1px solid rgba(249,115,22,.15)">' + lbl + '</div>'); }
    ...
    dToggle('input_boolean.frarik_tostapane_notify_push',   '📱 Push');
    dToggle('input_boolean.frarik_tostapane_notify_google', '🔊 Google');
    dToggle('input_boolean.frarik_tostapane_notify_alexa',  '🗣 Alexa');
    dTime('input_datetime.frarik_tostapane_orario_inizio_notifiche', '⏰ Orario inizio notifiche');
    ...
    dToggle('automation.frarik_tostapane_off_automatico', 'Auto OFF abilitato');
    dTime('input_datetime.frarik_tostapane_off_automatico', 'Orario spegnimento');
    dSec('📝 Personalizzazione');
    dText('input_text.frarik_tostapane_nome',      'Nome elettrodomestico');
    dText('input_text.frarik_tostapane_messaggio', 'Messaggio notifica');
    dNum('input_number.costo_energia', 'Costo energia', '€/kWh', 0, 2, 0.001);
    dInfo('Ultimo reset contatori', ss('input_text.frarik_tostapane_data_reset'));

    const swCss = '<style>.fi-sw{width:44px;height:26px;border-radius:13px;cursor:pointer;position:relative;flex-shrink:0;transition:background .25s}.fi-sw.on{background:#f97316}.fi-sw.off{background:rgba(255,255,255,.12)}.fi-knob{position:absolute;top:3px;width:20px;height:20px;border-radius:50%;background:#fff;transition:left .25s;box-shadow:0 1px 4px rgba(0,0,0,.4)}.fi-sw.on .fi-knob{left:21px}.fi-sw.off .fi-knob{left:3px}.fi-inp:focus{border-color:rgba(249,115,22,.55)!important;box-shadow:0 0 0 2px rgba(249,115,22,.12)}</style>';
    const saveBtn = '<button id="fi-save" style="width:100%;margin-top:12px;padding:13px;border-radius:12px;background:rgba(249,115,22,.15);border:1px solid rgba(249,115,22,.4);color:#f97316;font-size:14px;font-weight:700;cursor:pointer">💾 Salva impostazioni</button>';
    ...
    const ov = mkOv(popShell('⚙', '100,116,139', 'Impostazioni', S(h, 'input_text.frarik_tostapane_nome') || c.name || 'Tostapane', closeId, swCss + rows.join('') + saveBtn + resetBtn), closeId);
    ...
    if (sb) sb.addEventListener('click', function() {
      ov.querySelectorAll('.fi-sw[data-entity]').forEach(function(sw) {
        callSvc(sw.dataset.entity.split('.')[0], sw.classList.contains('on') ? 'turn_on' : 'turn_off', {entity_id: sw.dataset.entity});
      });
      ov.querySelectorAll('.fi-inp[data-entity]').forEach(function(inp) {
        const entity = inp.dataset.entity, type = inp.dataset.svctype;
        if (!entity) return;
        if (type === 'time') { if (inp.value) callSvc('input_datetime', 'set_datetime', {entity_id: entity, time: inp.value + ':00'}); }
        else if (type === 'number') { const v = parseFloat(inp.value); if (!isNaN(v)) { callSvc('input_number', 'set_value', {entity_id: entity, value: v}); if (entity === c.pk_soglia) try { localStorage.setItem('_fsg_' + c.pk_soglia, v); } catch(e) {} } }
        else if (type === 'text') { callSvc('input_text', 'set_value', {entity_id: entity, value: inp.value}); }
      });
    ...
    if (rb) rb.addEventListener('click', function() {
      callSvc('script', 'turn_on', {entity_id: 'script.frarik_tostapane_reset_sensori'});
  ...
  function update(card, hass, el) {
    const h = H(), c = cfgFor(card);
    const sig = [CARD.version, S(h, c.pk_power), S(h, c.pk_running), S(h, c.pk_kwh_oggi), S(h, c.pk_cicli_oggi), Attr(h, c.pk_time_on, 'Oggi'), Attr(h, c.pk_time_on, 'tempo_ciclo_tostapane'), Attr(h, c.pk_time_on, 'costo_oggi_tostapane')].join('|');
  ...
  /* ── PKG YAML EMBEDDED ── */
  var _TOSTAPANE_PKG_YAML = `###############################################################
#   Package: Frarik — Centro Controllo Tostapane             #
...
homeassistant:
  customize:
    package.node_anchors:
      customize: &customize
        package: 'Frarik — Centro Controllo Tostapane 2.0'
      setting:
        Sensore Potenza Tostapane: &sensore_potenza   "{{ states('IL_TUO_SENSORE_POTENZA') | float(0) }}"
        Switch Tostapane:          &switch_tostapane   "IL_TUO_SWITCH"
...
notify:
  - name: frarik_tostapane_notify
    platform: group
    services: *push

sensor:
  - platform: integration
    source: sensor.frarik_tostapane_potenza_w
    name: frarik_tostapane_kwh
    unit_prefix: k
    method: left
    round: 2
...
input_number:
  ...
  frarik_tostapane_soglia_w:
    name: Soglia Lavoro Tostapane W
    icon: mdi:flash
    min: 0
    max: 5000
    step: 1.00
  ...
  frarik_tostapane_tempo_innesco_m:
    name: Tempo Innesco Tostapane M
    icon: mdi:timer
    min: 0
    max: 60
    step: 1.00
  ...
  frarik_tostapane_avvio_ritardato_s:
    name: Avvio Ritardato Tostapane S
    icon: mdi:timer-sand
    min: 0
    max: 60
    step: 1.00
  ...
  frarik_tostapane_consumo_lunedi:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_consumo_martedi:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_costo_martedi:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_consumo_mercoledi:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_costo_mercoledi:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_consumo_giovedi:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_costo_giovedi:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_consumo_venerdi:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_costo_venerdi:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_consumo_sabato:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_consumo_domenica:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
  ...
  frarik_tostapane_costo_domenica:
    icon: mdi:counter
    min: 0
    max: 999999
    mode: box
...
input_text:
  ...
  frarik_tostapane_nome:
    name: Nome Tostapane
    icon: mdi:rename-box
    max: 50

  frarik_tostapane_messaggio:
    name: Messaggio Notifica Tostapane
    icon: mdi:message-text
    max: 255

  frarik_tostapane_data_reset:
    name: Data Reset Tostapane
    icon: mdi:calendar-refresh
    max: 20

  frarik_tostapane_cicli_lunedi:
    name: Cicli Lun Tostapane
    icon: mdi:counter
    max: 10

  frarik_tostapane_cicli_martedi:
    name: Cicli Mar Tostapane
    icon: mdi:counter
    max: 10

  frarik_tostapane_cicli_mercoledi:
    name: Cicli Mer Tostapane
    icon: mdi:counter
    max: 10

  frarik_tostapane_cicli_giovedi:
    name: Cicli Gio Tostapane
    icon: mdi:counter
    max: 10

  frarik_tostapane_cicli_venerdi:
    name: Cicli Ven Tostapane
    icon: mdi:counter
    max: 10

  frarik_tostapane_cicli_sabato:
    name: Cicli Sab Tostapane
    icon: mdi:counter
    max: 10

  frarik_tostapane_cicli_domenica:
    name: Cicli Dom Tostapane
    icon: mdi:counter
    max: 10

  frarik_tostapane_tempo_lunedi:
    name: Tempo Lun Tostapane
    icon: mdi:timer
    max: 20

  frarik_tostapane_tempo_martedi:
    name: Tempo Mar Tostapane
    icon: mdi:timer
    max: 20

  frarik_tostapane_tempo_mercoledi:
    name: Tempo Mer Tostapane
    icon: mdi:timer
    max: 20

  frarik_tostapane_tempo_giovedi:
    name: Tempo Gio Tostapane
    icon: mdi:timer
    max: 20

  frarik_tostapane_tempo_venerdi:
    name: Tempo Ven Tostapane
    icon: mdi:timer
    max: 20

  frarik_tostapane_tempo_sabato:
    name: Tempo Sab Tostapane
    icon: mdi:timer
    max: 20

  frarik_tostapane_tempo_domenica:
    name: Tempo Dom Tostapane
    icon: mdi:timer
    max: 20

input_boolean:

  frarik_tostapane_notify_push:
    name: Notifica Push Tostapane
    icon: mdi:cellphone

  frarik_tostapane_notify_google:
    name: Notifica Google Tostapane
    icon: mdi:google-assistant

  frarik_tostapane_notify_alexa:
    name: Notifica Alexa Tostapane
    icon: mdi:amazon-alexa

  frarik_tostapane_switch:
    name: Switch Tostapane
    icon: mdi:power-socket-it

input_datetime:

  frarik_tostapane_orario_inizio_notifiche:
    name: Orario Inizio Notifiche Tostapane
    has_date: false
    has_time: true

  frarik_tostapane_orario_fine_notifiche:
    name: Orario Fine Notifiche Tostapane
    has_date: false
    has_time: true

  frarik_tostapane_off_automatico:
    name: Orario Off Automatico Tostapane
    has_date: false
    has_time: true

counter:

  frarik_tostapane_cicli_totale:
    name: Cicli Totale Tostapane
    icon: mdi:counter
    step: 1
    restore: true
...
template:
  - sensor:
      - name: frarik_tostapane_potenza_w
        unit_of_measurement: "W"
        device_class: power
        state: *sensore_potenza

      - name: frarik_tostapane_versione
        state: "2.0"

      - name: frarik_tostapane_cicli_oggi
        state: >-
          {{ states('counter.frarik_tostapane_cicli_totale') }}

      - name: frarik_tostapane_cicli_mese
        state: >-
          {{ states('counter.frarik_tostapane_cicli_totale') }}

      - name: frarik_tostapane_cicli_anno
        state: >-
          {{ states('counter.frarik_tostapane_cicli_totale') }}

      - name: frarik_tostapane_time_on
        state: >-
          {{ 'on' if states('binary_sensor.frarik_tostapane_motore') == 'on' else 'off' }}
        attributes:
          terminato: >-
            {{ now().strftime('%H:%M') }}
          Oggi: "—"
          ...
          Mese Precedente: "—"
          Anno: "—"
          tempo_ciclo_tostapane: "—"
          consumo_ciclo_tostapane: "—"
          costo_ciclo_tostapane: 0
          costo_oggi_tostapane: >-
            {{ (states('sensor.frarik_tostapane_energy_oggi') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_ieri_tostapane: 0
          costo_mese_tostapane: >-
            {{ (states('sensor.frarik_tostapane_energy_mese') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_mese_precedente_tostapane: 0
          costo_anno_tostapane: >-
            {{ (states('sensor.frarik_tostapane_energy_anno') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_anno_precedente_tostapane: 0

  - binary_sensor:
      - name: frarik_tostapane_motore
        device_class: running
        state: >-
          {{ states('IL_TUO_SENSORE_POTENZA') | float(0) >= states('input_number.frarik_tostapane_soglia_w') | float(500) }}

  - sensor:
      - name: frarik_tostapane_energy_oggi
        unit_of_measurement: "kWh"
        device_class: energy
        state: >-
          {{ states('sensor.frarik_tostapane_kwh') | float(0) | round(3) }}

      - name: frarik_tostapane_energy_mese
        unit_of_measurement: "kWh"
        device_class: energy
        state: >-
          {{ states('sensor.frarik_tostapane_kwh') | float(0) | round(3) }}

      - name: frarik_tostapane_energy_anno
        unit_of_measurement: "kWh"
        device_class: energy
        state: >-
          {{ states('sensor.frarik_tostapane_kwh') | float(0) | round(3) }}
...
automation:
  ...
  - alias: frarik_tostapane_off_automatico
    id: frarik_tostapane_off_automatico
    trigger:
      - platform: time
        at: input_datetime.frarik_tostapane_off_automatico
    condition:
      - condition: state
        entity_id: input_boolean.frarik_tostapane_switch
        state: 'on'
    action:
      - service: switch.turn_off
        target:
          entity_id: *switch_tostapane

  - alias: frarik_tostapane_notifica_fine_ciclo
    id: frarik_tostapane_notifica_fine_ciclo
    trigger:
      - platform: state
        entity_id: binary_sensor.frarik_tostapane_motore
        from: 'on'
        to: 'off'
        for:
          seconds: "{{ states('input_number.frarik_tostapane_avvio_ritardato_s') | int(30) }}"
    condition:
      - condition: time
        after: input_datetime.frarik_tostapane_orario_inizio_notifiche
        before: input_datetime.frarik_tostapane_orario_fine_notifiche
    action:
      - choose:
          - conditions:
              - condition: state
                entity_id: input_boolean.frarik_tostapane_notify_push
                state: 'on'
            sequence:
              - service: frarik_tostapane_notify.send_message
                data:
                  message: "{{ states('input_text.frarik_tostapane_messaggio') or 'Tostapane terminato!' }}"
                  title: "🍞 Tostapane"
      - service: counter.increment
        target:
          entity_id: counter.frarik_tostapane_cicli_totale

  - alias: frarik_tostapane_reset_settimanale
    id: frarik_tostapane_reset_settimanale
    trigger:
      - platform: time
        at: "00:01:00"
    action:
      - service: input_number.set_value
        target:
          entity_id: >
            ...
            {{ {'Monday':'input_number.frarik_tostapane_consumo_lunedi','Tuesday':'input_number.frarik_tostapane_consumo_martedi','Wednesday':'input_number.frarik_tostapane_consumo_mercoledi','Thursday':'input_number.frarik_tostapane_consumo_giovedi','Friday':'input_number.frarik_tostapane_consumo_venerdi','Saturday':'input_number.frarik_tostapane_consumo_sabato','Sunday':'input_number.frarik_tostapane_consumo_domenica'}[g] }}
        data:
          value: "{{ states('sensor.frarik_tostapane_energy_oggi') }}"
      - service: input_number.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_number.frarik_tostapane_costo_lunedi','Tuesday':'input_number.frarik_tostapane_costo_martedi','Wednesday':'input_number.frarik_tostapane_costo_mercoledi','Thursday':'input_number.frarik_tostapane_costo_giovedi','Friday':'input_number.frarik_tostapane_costo_venerdi','Saturday':'input_number.frarik_tostapane_costo_sabato','Sunday':'input_number.frarik_tostapane_costo_domenica'}[g] }}
        data:
          value: "{{ state_attr('sensor.frarik_tostapane_time_on','costo_oggi_tostapane') }}"

script:

  frarik_tostapane_reset_sensori:
    alias: Reset Sensori Tostapane
    sequence:
      ...
      - service: input_text.set_value
        target:
          entity_id: input_text.frarik_tostapane_data_reset
        data:
          value: "{{ now().strftime('%d/%m/%Y %H:%M') }}"
`;

  /* ── PKG BUILD ── */
  var _TST_WIZ_KEY = 'frarik_pkg_wizard_tostapane';

  function _buildPkgTST(potenza, sw, push, google, alexa, _tpl) {
    var ind = '          ';
    ...
    var yaml = (_tpl || _TOSTAPANE_PKG_YAML)
  ...
  /* ── WIZARD ── */
  function _openWizardTST(hass, onDone, _tpl, opts) {
    ...
    var saved = null;
    try { saved = JSON.parse(localStorage.getItem(_TST_WIZ_KEY) || 'null'); } catch(e) {}
    ...
    function renderWiz() {
      sr.innerHTML = '<style>'
        ...
        + '<div><div class="wd-tit">' + (isUpdate ? 'Aggiorna PKG Tostapane' : 'Installa PKG Tostapane') + '</div><div class="wd-sub">frarik_tostapane.yaml → config/packages/</div></div>'
        + '<button class="wd-x" id="wd-x">✕</button></div>'
        + '<div class="wd-body">'
        + '<div><div class="wd-sec">Sensori</div>'
        + '<div class="wd-lbl">Sensore Potenza (W)</div>'
        + '<div class="wd-frow"><input class="wd-inp" id="f-potenza" type="text" autocomplete="off" placeholder="sensor.presa_tostapane_power" value="' + ((saved && saved.potenza) || '').replace(/"/g, '&quot;') + '"><div class="wd-drop" id="d-potenza"></div></div>'
        + '<div class="wd-lbl">Switch Presa</div>'
        + '<div class="wd-frow"><input class="wd-inp" id="f-switch" type="text" autocomplete="off" placeholder="switch.presa_tostapane" value="' + ((saved && saved.sw) || '').replace(/"/g, '&quot;') + '"><div class="wd-drop" id="d-switch"></div></div>'
        + '</div>'
      ...
      sr.getElementById('wd-install').addEventListener('click', function() {
        ...
        try { localStorage.setItem(_TST_WIZ_KEY, JSON.stringify({potenza: potenza, sw: sw, push: push, google: google, alexa: alexa})); } catch(e) {}
        var yaml = _buildPkgTST(potenza, sw, push, google, alexa, _tpl);
        ...
        fetch(base + '/api/frarik/pkg/install', {
          ...
          body: JSON.stringify({name: 'frarik/frarik_tostapane.yaml', content: yaml})
        }).then(function(r) { return r.json().then(function(j) { return {r: r, j: j}; }); })
          .then(function(res) {
            destroy();
            if (res.r.ok && res.j.ok) {
              try { if (typeof window.showToast === 'function') window.showToast(isUpdate ? '🔄 PKG Tostapane aggiornato! Riavvia HA.' : '📦 PKG Tostapane installato! Riavvia HA.'); } catch(e) {}
  ...
  /* ── CARD REGISTRATION ── */
  const CARD = {
    id: 'tostapane', name: 'Tostapane', icon: '🍞', version: '2.3',
    ...
    frarik_pkg_check: 'sensor.frarik_tostapane_versione',
    frarik_pkg_id: 'frarik_tostapane',
    frarik_pkg_version: '2.0',
    openWizard: _openWizardTST,
    _buildPkgFromConfig: function(cfg, _tpl) { return _buildPkgTST(cfg.potenza || '', cfg.sw || '', cfg.push || [], cfg.google || [], cfg.alexa || [], _tpl); },
  };
  ...
  try { console.log('[FratechStore] Card registrata: tostapane v' + CARD.version); } catch(e) {}
