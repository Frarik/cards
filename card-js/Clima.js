...
(function () {
  ...
  // Stato sezione aperta persistito per card (sopravvive ai re-render da update HA)
  var _openSecs = {};

  /* ── Air flow: 10 stream con delay uniformi → niente scatti ── */
  function airStreams(rid, col, show) {
    if (!show) return '';
    const dur = 3.8;
    const positions = [
      [3,72],[16,56],[7,76],[25,60],[11,50],
      [30,66],[5,58],[21,72],[27,56],[9,64]
    ];
    return positions.map(function(s, i) {
      const delay = (i * dur / positions.length).toFixed(2);
  ...
  function render(card) {
    ...
    /* ── CSS ── */
    const css = '<style>'
      // Aria: lineare per scorrimento uniforme, fade in e out sui bordi
      +'@keyframes '+rid+'air{'
      ...
      +'}'
      // Aletta: lenta come un vero split (5 s), angolo dolce
      +'@keyframes '+rid+'flap{'
        +'0%{transform:rotateX(3deg)}'
        +'50%{transform:rotateX(34deg)}'
      ...
      +'#'+rid+' .tog{flex:1;padding:8px 4px;border-radius:12px;border:1px solid rgba(255,255,255,.1);cursor:pointer;font-size:10px;font-weight:700;'
        +'display:flex;flex-direction:column;align-items:center;gap:3px;background:rgba(255,255,255,.07);color:#94a3b8;transition:all .2s;}'
      +'#'+rid+' .tog:hover{background:rgba(255,255,255,.12);color:#e2e8f0;}'
      +'#'+rid+' [data-secpanel]{display:none;flex-direction:column;gap:6px;}'
      +'</style>';

    /* ── Header: nome centrato + badge sensori top-right ── */
    const sensorBadges = (sensorT || sensorH)
      ? '<div style="display:flex;gap:5px;flex-shrink:0">'
          +(sensorT?'<div style="font-size:11px;font-weight:700;color:#94a3b8;background:rgba(255,255,255,.07);border-radius:7px;padding:3px 7px">🌡 '+sensorT+'°</div>':'')
          +(sensorH?'<div style="font-size:11px;font-weight:700;color:#64748b;background:rgba(255,255,255,.07);border-radius:7px;padding:3px 7px">💧 '+sensorH+'%</div>':'')
        +'</div>'
      : '<div></div>';

    const header = '<div style="display:flex;align-items:center">'
      +'<div style="flex:1"></div>'
      +'<div style="font-size:19px;font-weight:800;letter-spacing:-.2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+nm+'</div>'
      +'<div style="flex:1;display:flex;justify-content:flex-end">'+sensorBadges+'</div>'
      +'</div>';

    /* ── Corpo AC ── */
    const acBody = '<div style="position:relative">'

      // Guscio bianco (overflow:hidden per border-radius)
      +'<div style="border-radius:14px;overflow:hidden;'
        ...
        +'<div style="display:flex;align-items:center;gap:8px;padding:5px 12px;background:rgba(0,0,0,.04);border-bottom:1px solid rgba(0,0,0,.06)">'
          ...
          +'</div>'
          +(currentTemp?'<div style="font-size:10px;font-weight:700;color:rgba(0,0,0,.4);background:rgba(0,0,0,.06);padding:2px 7px;border-radius:99px">🌡 '+currentTemp+'°C</div>':'')
        +'</div>'

        // Faccia principale: sinistra bianca + separatore + display LED (più piccolo: 108px)
        +'<div style="display:flex;height:78px">'

          // Area sinistra bianca pulita
      ...
      // Aletta (fuori dall'overflow:hidden, slow 3D rotation)
      +'<div style="position:absolute;bottom:-7px;left:14px;right:14px;height:13px;perspective:220px;perspective-origin:50% 0%">'
        +'<div style="width:100%;height:100%;'
          +'background:linear-gradient(180deg,rgba(255,255,255,.25) 0%,rgba(198,210,224,.92) 40%,rgba(152,170,194,.85) 100%);'
    ...
    /* ── Pannelli sezione (nascosti, mostrati dal toggle) ── */
    ...
    const panelWrap = 'background:rgba(255,255,255,.04);border-radius:13px;padding:10px;border:1px solid rgba(255,255,255,.08);';

    const modePanel = '<div data-secpanel="mode" style="'+panelWrap+'">'
      +'<div style="font-size:9px;font-weight:800;color:rgba(255,255,255,.3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Modalità</div>'
    ...
    const fanPanel = fanModes.length>1
      ? '<div data-secpanel="fan" style="'+panelWrap+'">'
          +'<div style="font-size:9px;font-weight:800;color:rgba(255,255,255,.3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Ventola</div>'
          +'<div style="display:flex;gap:5px;flex-wrap:wrap">'
    ...
    const swingPanel = swingModes.length>1
      ? '<div data-secpanel="swing" style="'+panelWrap+'">'
          +'<div style="font-size:9px;font-weight:800;color:rgba(255,255,255,.3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Oscillazione</div>'
          +'<div style="display:flex;gap:5px;flex-wrap:wrap">'
  ...
  function mount(card, hass, el) {
    ...
    el.addEventListener('click', function(e) {
      ...
      // Toggle sezione dropdown (no debounce)
      if (action === 'toggle') {
        const sec   = btn.getAttribute('data-sec');
        const secEl = el.querySelector('[data-secpanel="'+sec+'"]');
        if (!secEl) return;
        const wasOpen = secEl.style.display !== 'none';
        // Chiudi tutto
      ...
      // Debounce 700ms per chiamate servizio
      const now = Date.now();
      ...
      if (action === 'mode') {
        ...
        else { callSvc('climate','set_hvac_mode',{entity_id:entityId,hvac_mode:val}); const sv=load(card); sv.lastMode=val; save(card,sv); }
      } else if (action === 'fan') {
  ...
  /* ── UPDATE: ripristina sezione aperta dopo re-render ── */
  ...
  function openCfg(card, el) {
    ...
    ov.innerHTML = '<div style="width:min(460px,94vw);max-height:90vh;overflow-y:auto;background:#0b1220;border:1px solid rgba(255,255,255,.14);border-radius:18px;box-shadow:0 30px 80px rgba(0,0,0,.6);padding:20px;color:#f1f5f9">'
      ...
      // Entità clima
      +'<div style="margin-bottom:10px;position:relative"><label style="'+stLbl+'">Entità Clima <span style="font-weight:400;color:#475569;font-size:10px;text-transform:none;letter-spacing:0">climate.xxx</span></label>'
        +'<input id="cl-ent" type="text" value="'+(c.entity||'').replace(/"/g,'&quot;')+'" autocomplete="off" placeholder="Clicca o scrivi…" style="'+stInp+'">'
        +'<div id="cl-ent-d" style="'+stDrop+'"></div>'
      +'</div>'

      // Sensore temperatura
      +'<div style="margin-bottom:10px;position:relative"><label style="'+stLbl+'">Sensore Temperatura <span style="font-weight:400;color:#475569;font-size:10px;text-transform:none;letter-spacing:0">opzionale</span></label>'
        ...
        +'<div id="cl-temp-d" style="'+stDrop+'"></div>'
      +'</div>'

      // Sensore umidità
      +'<div style="margin-bottom:10px;position:relative"><label style="'+stLbl+'">Sensore Umidità <span style="font-weight:400;color:#475569;font-size:10px;text-transform:none;letter-spacing:0">opzionale</span></label>'
        +'<input id="cl-hum" type="text" value="'+(c.humEntity||'').replace(/"/g,'&quot;')+'" autocomplete="off" placeholder="sensor.umidita_salotto" style="'+stInp+'">'
        +'<div id="cl-hum-d" style="'+stDrop+'"></div>'
      +'</div>'
    ...
    /* helper combobox */
    function makeCombo(inpId, dropId, defaultList) {
      var inp  = ov.querySelector('#'+inpId);
      var drop = ov.querySelector('#'+dropId);
      function show() {
  ...
  var CARD = {
    id:'clima-card', name:'Climatizzatore', icon:'❄️', version:'2.2',
    desc:'Split murale: aletta 3D lenta, aria fluida colorata, badge temp/umidità, 3 menu dropdown per modalità/ventola/alette.',
    colSpan:2, rowSpan:4,
  ...
  try{ console.log('[FratechStore] Card registrata: clima-card v2.2'); }catch(e){}
})();
