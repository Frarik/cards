...
(function () {
  ...
  function mColor(mode) { return HVAC_COL[mode] || '#475569'; }
  function mLabel(mode) { return HVAC_LBL[mode] || mode; }
  function mIcon(mode)  { return HVAC_ICO[mode] || '⏻'; }
  function fLabel(m)    { return FAN_LBL[m]   || m; }
  function sLabel(m)    { return SWING_LBL[m] || m; }

  /* ── Air flow streams ── */
  function airStreams(rid, col, isActive) {
    if (!isActive) return '';
    const def = [
      [4,72,0,2.2],[16,56,0.38,1.9],[8,78,0.72,2.4],[24,62,1.15,2.0],
      [12,50,0.55,1.85],[30,68,1.45,2.45],[6,60,0.92,1.75],[20,74,0.2,2.3],
      [27,58,1.65,2.05],[10,65,1.85,1.65],
    ];
  ...
  function render(card) {
    ...
    // Read modes FROM entity attributes
    ...
    /* ── CSS ── */
    const css = '<style>'
      +'@keyframes '+rid+'air{0%{transform:translateY(0);opacity:0}14%{opacity:.72}82%{opacity:.45}100%{transform:translateY(66px);opacity:0}}'
      +'@keyframes '+rid+'flap{0%,100%{transform:perspective(180px) rotateX(4deg)}50%{transform:perspective(180px) rotateX(48deg)}}'
      +'@keyframes '+rid+'led{0%,100%{opacity:1}46%{opacity:.7}}'
      +'@keyframes '+rid+'ind{0%,100%{opacity:.8}50%{opacity:1;box-shadow:0 0 14px '+mCol+',0 0 26px '+mCol+'77}}'
      ...
      +'#'+rid+' .cb:hover{filter:brightness(1.25);}'
      +'</style>';
    ...
    const acBody = ''
      ...
      +'background:linear-gradient(170deg,#ffffff 0%,#f4f7fc 70%,#eaeff7 100%);'
      +'box-shadow:0 6px 28px rgba(0,0,0,.45),'
      +'inset 0 1px 0 rgba(255,255,255,.95),'
      +'inset 0 -2px 5px rgba(0,0,0,.1),'
      +'inset 2px 0 0 rgba(255,255,255,.6),'
      +'inset -2px 0 0 rgba(0,0,0,.05);">'

        // ── Status bar (top)
        +'<div style="display:flex;align-items:center;gap:8px;padding:7px 14px;background:rgba(0,0,0,.04);border-bottom:1px solid rgba(0,0,0,.06)">'
          +'<div style="width:8px;height:8px;border-radius:50%;flex-shrink:0;'
            +'background:'+(isOn?mCol:'#9ca3af')+';'
            +'box-shadow:'+(isOn?'0 0 8px '+mCol:'none')+';'
            +(isOn?'animation:'+rid+'ind 2.4s ease-in-out infinite;':'')
          +'"></div>'
          +'<div style="font-size:11px;font-weight:700;color:rgba(0,0,0,.45);text-transform:uppercase;letter-spacing:.06em;flex:1">'
            +(isOn ? mIcon(mode)+' '+mLabel(mode) : 'Spento')
          +'</div>'
          // Current room temp badge
          +(currentTemp ? '<div style="font-size:11px;font-weight:700;color:rgba(0,0,0,.5);background:rgba(0,0,0,.07);padding:2px 8px;border-radius:99px">🌡 '+currentTemp+'°C</div>' : '')
        +'</div>'

        // ── Main face: clean white with LED display
        +'<div style="padding:12px 16px;display:flex;align-items:center;justify-content:space-between;gap:10px">'

          // Left: mode icon big
          +'<div style="font-size:36px;opacity:'+(isOn?'1':'.2')+';transition:opacity .4s">'
            +mIcon(mode)
          +'</div>'

          // Center: LED display
          +'<div style="flex:1;display:flex;justify-content:center">'
            +'<div style="background:#060e0b;border-radius:10px;padding:6px 16px;'
            +'border:1px solid rgba(0,0,0,.45);'
            +'box-shadow:inset 0 3px 10px rgba(0,0,0,.65),inset 0 0 22px rgba(0,0,0,.35)">'
              // Temperature digits
              +'<div style="font-family:\'Courier New\',Courier,monospace;font-size:36px;font-weight:700;letter-spacing:2px;line-height:1;text-align:center;'
              +'color:'+(isOn?mCol:'#0d1a12')+';'
              +'text-shadow:'+(isOn?'0 0 16px '+mCol+',0 0 32px '+mCol+'66':'none')+';'
              +(isOn?'animation:'+rid+'led 3s ease-in-out infinite;':'')
              +'">'+targetTemp+'</div>'
              // °C label
              +'<div style="font-size:10px;font-family:monospace;letter-spacing:2px;text-align:right;margin-top:2px;'
              +'color:'+(isOn?mCol+'aa':'#0d1a12')+'">'
                +(isOn?'°C — '+mLabel(mode):'OFF')
              +'</div>'
            +'</div>'
          +'</div>'

          // Right: fan icon
          +'<div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:'+(isOn?'1':'.2')+';">'
            +'<div style="font-size:22px;'+(mode==='fan_only'&&isOn?'animation:'+rid+'spin 1.4s linear infinite;display:inline-block':'')+'">💨</div>'
            +'<div style="font-size:9px;font-weight:700;color:rgba(0,0,0,.35);text-transform:uppercase">'+fLabel(fanMode)+'</div>'
          +'</div>'

        +'</div>'

        // ── Bottom: vent + animated flap
        +'<div style="position:relative;height:20px;'
        +'background:linear-gradient(180deg,rgba(0,0,0,.04),rgba(0,0,0,.09));'
        +'border-top:1px solid rgba(0,0,0,.07)">'
          // Animated flap
          +'<div style="position:absolute;left:14px;right:14px;bottom:2px;height:12px;'
          +'background:linear-gradient(180deg,rgba(255,255,255,.25) 0%,rgba(0,0,0,.16) 100%);'
          +'border-radius:6px 6px 3px 3px;'
          +'transform-origin:50% 0%;'
          +'box-shadow:0 2px 6px rgba(0,0,0,.22),inset 0 1px 0 rgba(255,255,255,.4);'
    ...
    /* ── Air flow ── */
    const airSection = '<div style="position:relative;height:'+(isActive?'64px':'6px')+';overflow:hidden;transition:height .5s ease;margin-top:-2px;pointer-events:none">'
      +airStreams(rid, mCol, isActive)
    +'</div>';

    /* ── Selector button helper ── */
    function selBtn(eId, action, val, label, isActive, col2) {
      const aStyle = isActive
        ? 'background:'+col2+'33;color:'+(col2||'#e2e8f0')+';border:1px solid '+col2+'66;box-shadow:0 0 10px '+col2+'44;'
        : 'background:rgba(255,255,255,.07);color:#64748b;border:1px solid rgba(255,255,255,.1);';
      return '<button class="cb" data-cid="'+eId+'" data-action="'+action+'" data-val="'+val+'" style="'+aStyle+'">'+label+'</button>';
    }
    ...
    // Mode selector (all hvac_modes from entity)
    const modeRow = '<div style="display:flex;flex-direction:column;gap:4px">'
      ...
      +hvacModes.map(function(m) {
        return selBtn(entityId, 'mode', m, mIcon(m)+' '+mLabel(m), mode===m, mColor(m));
      }).join('')
      +'</div></div>';
    ...
    const fanRow = fanModes.length > 1
      ? '<div style="display:flex;flex-direction:column;gap:4px">'
        ...
        +'<div style="display:flex;gap:5px;flex-wrap:wrap">'
        +fanModes.map(function(m) {
          return selBtn(entityId, 'fan', m, fLabel(m), fanMode===m, '#38bdf8');
        }).join('')
        +'</div></div>'
      : '';
    ...
    const swingRow = swingModes.length > 1
      ? '<div style="display:flex;flex-direction:column;gap:4px">'
        ...
        +'<div style="display:flex;gap:5px;flex-wrap:wrap">'
        +swingModes.map(function(m) {
          return selBtn(entityId, 'swing', m, sLabel(m), swingMode===m, '#a78bfa');
        }).join('')
        +'</div></div>'
      : '';
  ...
  function mount(card, hass, el) {
    ...
    el.addEventListener('click', function(e) {
      ...
      if (action === 'mode') {
        callSvc('climate', val==='off' ? 'turn_off' : 'set_hvac_mode', val==='off'
          ? { entity_id: entityId }
          : { entity_id: entityId, hvac_mode: val });
        if (val !== 'off') { const sv=load(card); sv.lastMode=val; save(card,sv); }
      } else if (action === 'fan') {
  ...
  function openCfg(card, el) {
    ...
    function comboField(fid, lbl2, val, hint, ids) {
    ...
    ov.innerHTML = '<div style="width:min(440px,94vw);background:#0b1220;border:1px solid rgba(255,255,255,.14);border-radius:18px;box-shadow:0 30px 80px rgba(0,0,0,.6);padding:20px;color:#f1f5f9">'
      ...
      +'<div style="margin-bottom:10px"><label style="'+stLbl+'">Nome visualizzato (centrato)</label>'
      +'<input id="cl-name" type="text" value="'+(c.name||'').replace(/"/g,'&quot;')+'" placeholder="es. CLIMA SALOTTO" style="'+stBase+'"></div>'
      +comboField('cl-ent', 'Entità climatizzatore', c.entity||'', 'climate.xxx', climIds)
  ...
  var CARD = {
    id:'clima-card', name:'Climatizzatore', icon:'❄️', version:'2.0',
    desc:'Split murale: corpo bianco pulito, display LED, alette 3D, flusso aria colorato per modalità, selettori hvac/fan/swing da attributi entità.',
    colSpan:2, rowSpan:4,
  ...
  try { console.log('[FratechStore] Card registrata: clima-card v2.0'); } catch(e) {}
})();
