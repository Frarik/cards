...
(function () {
  ...
  function render(card) {
    ...
    const barMax = 7200; const barPct = Math.min(100, (pw / barMax) * 100);
    const barCol = pw < 50 ? '#64748b' : pw <= 1800 ? '#22c55e' : pw <= 3600 ? '#eab308' : pw <= 5400 ? '#f97316' : '#ef4444';
    ...
    return css
      ...
      + '<div class="fc-hdr-tit">' + (S(h, 'input_text.frarik_induzione_nome') || c.name || 'Induzione') + '</div>'
  ...
  function openEnergiaPopup(c) {
    ...
    const content = '<div style="background:rgba(56,189,248,.1);border-radius:12px;padding:12px 14px;text-align:center;margin-bottom:12px">'
      ...
      + row('Mese precedente', fmtEur(Attr(h, ton, 'costo_mese_precedente_induzione')), '#fff')
      + row('Questo anno', fmtEur(Attr(h, ton, 'costo_anno_induzione')), '#7dd3fc')
      + row('Anno precedente', fmtEur(Attr(h, ton, 'costo_anno_precedente_induzione')), '#fff');
  ...
  function openCicliPopup(c) {
    ...
    DAYS.forEach(function (d, i) {
      ...
      const cicli   = isToday ? cleanVal(S(h, c.pk_cicli_oggi))      : cleanVal(S(h, 'input_text.frarik_induzione_cicli_' + d));
      const tempo   = isToday ? cleanVal(Attr(h, ton, 'Oggi'))        : cleanVal(S(h, 'input_text.frarik_induzione_tempo_' + d));
      const kwRaw   = isToday ? S(h, c.pk_kwh_oggi) : S(h, 'input_number.frarik_induzione_consumo_' + d);
      const costRaw = isToday ? Attr(h, ton, 'costo_oggi_induzione') : S(h, 'input_number.frarik_induzione_costo_' + d);
  ...
  function openImpostazioniHAPopup(c) {
    ...
    dTime('input_datetime.frarik_induzione_orario_inizio_notifiche', '⏰ Orario inizio notifiche');
    dTime('input_datetime.frarik_induzione_orario_fine_notifiche',   '⏰ Orario fine notifiche');
    ...
    dTime('input_datetime.frarik_induzione_off_automatico', 'Orario spegnimento');
    ...
    const ov = mkOv(popShell('⚙', '100,116,139', 'Impostazioni', S(h, 'input_text.frarik_induzione_nome') || c.name || 'Induzione', closeId, swCss + rows.join('') + saveBtn + resetBtn), closeId);
...
homeassistant:
  customize:
    package.node_anchors:
      customize: &customize
        package: 'Frarik — Centro Controllo Induzione 2.0 — Frarik'
      setting:
...
####################################################
        ...
        Sensore Potenza Induzione: &sensore_potenza   "{{ states('IL_TUO_SENSORE_POTENZA') | float(0) }}"
      ...
      - name: frarik_induzione_time_on
        ...
        attributes:
          ...
          Mese Precedente: >
            ...
            {% endif %}
          Anno Precedente: >
            {% set hours = state_attr('sensor.frarik_induzione_tempo_anno', 'last_period') | float(0) %}
            {% set minutes = ((hours % 1) * 60) | int(0) %}
            {% set hours = (hours - (hours % 1)) | int(0) %}
            {% set day = ((hours | int(0) / 24)) | int(0) %}
            {% if day | int(0) > 0 %}
          ...
          consumo_ciclo_induzione: >-
            {{ (states('sensor.frarik_induzione_kwh') | float(0) - states('sensor.frarik_induzione_inizio_ciclo') | float(0)) | round(2) }} kWh
          costo_ciclo_induzione: >-
            {{ ((states('sensor.frarik_induzione_kwh') | float(0) - states('sensor.frarik_induzione_inizio_ciclo') | float(0)) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_oggi_induzione: >-
            {{ (states('sensor.frarik_induzione_energy_oggi') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_mese_induzione: >-
            {{ (states('sensor.frarik_induzione_energy_mese') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_anno_induzione: >-
            {{ (states('sensor.frarik_induzione_energy_anno') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_ieri_induzione: >-
            {{ (state_attr('sensor.frarik_induzione_energy_oggi', 'last_period') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_mese_precedente_induzione: >-
            {{ (state_attr('sensor.frarik_induzione_energy_mese', 'last_period') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_anno_precedente_induzione: >-
            {{ (state_attr('sensor.frarik_induzione_energy_anno', 'last_period') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
...
input_datetime:
  ...
  frarik_induzione_orario_inizio_notifiche:
    name: Orario Inizio Notifiche Induzione
    has_date: false
    has_time: true

  frarik_induzione_orario_fine_notifiche:
    name: Orario Fine Notifiche Induzione
    has_date: false
    has_time: true

  frarik_induzione_off_automatico:
    name: Induzione Off Automatico
    has_date: false
    has_time: true
...
- alias: frarik_induzione_automazione
  id: frarik_induzione_automazione
  max_exceeded: silent
  trigger:
    ...
    - conditions:
      ...
      - service: input_text.set_value
        target:
          entity_id: >
            ...
            {{ {'Monday':'input_text.frarik_induzione_cicli_lunedi','Tuesday':'input_text.frarik_induzione_cicli_martedi','Wednesday':'input_text.frarik_induzione_cicli_mercoledi','Thursday':'input_text.frarik_induzione_cicli_giovedi','Friday':'input_text.frarik_induzione_cicli_venerdi','Saturday':'input_text.frarik_induzione_cicli_sabato','Sunday':'input_text.frarik_induzione_cicli_domenica'}[g] }}
        data:
      ...
      - service: input_text.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_text.frarik_induzione_tempo_lunedi','Tuesday':'input_text.frarik_induzione_tempo_martedi','Wednesday':'input_text.frarik_induzione_tempo_mercoledi','Thursday':'input_text.frarik_induzione_tempo_giovedi','Friday':'input_text.frarik_induzione_tempo_venerdi','Saturday':'input_text.frarik_induzione_tempo_sabato','Sunday':'input_text.frarik_induzione_tempo_domenica'}[g] }}
        data:
      ...
      - service: input_number.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_number.frarik_induzione_consumo_lunedi','Tuesday':'input_number.frarik_induzione_consumo_martedi','Wednesday':'input_number.frarik_induzione_consumo_mercoledi','Thursday':'input_number.frarik_induzione_consumo_giovedi','Friday':'input_number.frarik_induzione_consumo_venerdi','Saturday':'input_number.frarik_induzione_consumo_sabato','Sunday':'input_number.frarik_induzione_consumo_domenica'}[g] }}
        data:
      ...
      - service: input_number.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_number.frarik_induzione_costo_lunedi','Tuesday':'input_number.frarik_induzione_costo_martedi','Wednesday':'input_number.frarik_induzione_costo_mercoledi','Thursday':'input_number.frarik_induzione_costo_giovedi','Friday':'input_number.frarik_induzione_costo_venerdi','Saturday':'input_number.frarik_induzione_costo_sabato','Sunday':'input_number.frarik_induzione_costo_domenica'}[g] }}
        data:
  ...
  - choose:
    - conditions:
      ...
      - service: input_text.set_value
        ...
        data:
          value: "{{ state_attr('sensor.frarik_induzione_time_on','tempo_ciclo_induzione') | trim }}"
    ...
    - choose:
      - conditions:
        - condition: trigger
          id: fine_ciclo
        - condition: time
          after: 'input_datetime.frarik_induzione_orario_inizio_notifiche'
          before: 'input_datetime.frarik_induzione_orario_fine_notifiche'
        - condition: state
        ...
        - service: tts.google_translate_say
          ...
          data:
            entity_id: *google
            message: >-
              {%- set dur=states('input_text.frarik_induzione_ultimo_ciclo')|trim -%}
              ...
              {{- states('input_text.frarik_induzione_messaggio') }} in {{ hv+sep+mv -}}
      ...
      - conditions:
        ...
        - repeat:
            ...
            - service: "notify.{{ repeat.item.service }}"
              continue_on_error: true
              data:
                title: "{{ states('input_text.frarik_induzione_nome') }}"
                message: >-
                  🫧 {{ states('input_text.frarik_induzione_messaggio') }}

                  ⏱ Ciclo durato: {{ states('input_text.frarik_induzione_ultimo_ciclo') | trim }}
...
- alias: frarik_induzione_off_automatico
  ...
  trigger:
    - platform: time
      at: 'input_datetime.frarik_induzione_off_automatico'
      id: auto_off
  condition: []
  action:
    - choose:
...
`;
  ...
  function _buildPkg(potenza, sw, push, google, alexa, _tpl) {
    ...
    var yaml = (_tpl || _INDUZIONE_PKG_YAML)
  ...
  function _openWizard(hass, onDone, _tpl, opts) {
    ...
    function renderWiz() {
      sr.innerHTML = '<style>'
        ...
        + '<div><div class="wd-tit">' + (isUpdate ? 'Aggiorna PKG Induzione' : 'Installa PKG Induzione') + '</div><div class="wd-sub">frarik_induzione.yaml → config/packages/</div></div>'
