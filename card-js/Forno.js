...
(function () {
  ...
  function keyOf(c) { return 'frarik_fornocard_' + (c.id || 'x'); }
  ...
  function _fornoSVG(running) {
    ...
    return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 108" style="display:block;width:100%;height:100%' + glow + '">'
      ...
      + '<rect x="2" y="19" width="68" height="5" fill="#070e1c"/>'
      + '<rect x="5" y="5" width="32" height="12" rx="3" fill="#020810" stroke="' + (running ? '#38bdf8' : '#162035') + '" stroke-width=".7"/>'
      + '<text x="21" y="13.5" text-anchor="middle" font-size="6.5" font-weight="bold" font-family="monospace" fill="' + (running ? '#38bdf8' : '#1a3050') + '">' + (running ? '200 C' : '-- --') + '</text>'
      + '<circle cx="51" cy="11" r="6" fill="#0a1830" stroke="' + c + '" stroke-width=".9"/>'
      + '<circle cx="51" cy="11" r="2.5" fill="' + (running ? '#0d2545' : '#060e1c') + '"/>'
      + '<line x1="51" y1="5.5" x2="51" y2="8.5" stroke="' + c + '" stroke-width="1" stroke-linecap="round"/>'
      + '<circle cx="64" cy="11" r="5" fill="#0a1830" stroke="' + c + '" stroke-width=".8"/>'
      + '<line x1="64" y1="6.5" x2="64" y2="9.5" stroke="' + c + '" stroke-width=".9" stroke-linecap="round"/>'
      + '<rect x="4" y="25" width="64" height="68" rx="5" fill="' + cf + '" stroke="' + c + '" stroke-width="1.1"/>'
      + '<rect x="8" y="29" width="56" height="46" rx="4" fill="' + (running ? 'rgba(56,189,248,.07)' : '#030912') + '" stroke="' + (running ? 'rgba(56,189,248,.3)' : '#1a3050') + '" stroke-width=".9"/>'
      + (running ? '<line x1="13" y1="36" x2="59" y2="36" stroke="#38bdf8" stroke-width="1.3" stroke-dasharray="6,3" style="animation:frnheat 1.5s ease-in-out infinite"/>' : '')
      + '<line x1="8" y1="52" x2="64" y2="52" stroke="' + c + '" stroke-width=".6" opacity="' + (running ? '.3' : '.1') + '"/>'
      + '<line x1="8" y1="62" x2="64" y2="62" stroke="' + c + '" stroke-width=".6" opacity="' + (running ? '.3' : '.1') + '"/>'
      + (running ? '<line x1="13" y1="71" x2="59" y2="71" stroke="rgba(56,189,248,.65)" stroke-width="1.3" stroke-dasharray="6,3" style="animation:frnheat 1.9s ease-in-out infinite .4s"/>' : '')
      + (running ? '<rect x="8" y="29" width="56" height="46" rx="4" fill="rgba(56,189,248,.06)" style="animation:frnglow 2s ease-in-out infinite"/>' : '')
      + '<rect x="12" y="95" width="48" height="5.5" rx="2.8" fill="' + c + '" opacity=".5"/>'
  ...
  function render(card) {
    ...
    const tempoC     = Attr(h, ton, 'tempo_ciclo_forno')   || '—';
    const consumoC   = Attr(h, ton, 'consumo_ciclo_forno') || '—';
    const costoC     = Attr(h, ton, 'costo_ciclo_forno');
    const kwOggi     = S(h, c.pk_kwh_oggi);
    const cicOggi    = S(h, c.pk_cicli_oggi);
    const timeOggi   = Attr(h, ton, 'Oggi')                || '—';
    const costoOggi  = Attr(h, ton, 'costo_oggi_forno');

    const pw     = pwV || 0;
    const col    = running ? '#38bdf8' : '#64748b';
    const statusLabel = running ? 'RESISTENZA ON' : 'STANDBY';
    const barMax = 3500; const barPct = Math.min(100, (pw / barMax) * 100);
    const barCol = pw < 50 ? '#64748b' : pw <= 900 ? '#22c55e' : pw <= 1800 ? '#eab308' : pw <= 2800 ? '#f97316' : '#ef4444';
    ...
    const heroHtml = '<div class="fc-hero">'
      + '<div class="fc-hero-img" data-sya="popup-cicli">' + _fornoSVG(running) + '</div>'
      + '<div class="fc-hero-r">'
    ...
    return css
      ...
      + '<div class="fc-hdr-tit">' + (S(h, 'input_text.frarik_forno_nome') || c.name || 'Forno') + '</div>'
  ...
  function openEnergiaPopup(c) {
    ...
    const content = '<div style="background:rgba(56,189,248,.1);border-radius:12px;padding:12px 14px;text-align:center;margin-bottom:12px">'
      ...
      + row('Oggi', fmtEur(Attr(h, ton, 'costo_oggi_forno')), '#7dd3fc')
      + row('Ieri', fmtEur(Attr(h, ton, 'costo_ieri_forno')), '#fff')
      + row('Questo mese', fmtEur(Attr(h, ton, 'costo_mese_forno')), '#7dd3fc')
      + row('Mese precedente', fmtEur(Attr(h, ton, 'costo_mese_precedente_forno')), '#fff')
      + row('Questo anno', fmtEur(Attr(h, ton, 'costo_anno_forno')), '#7dd3fc')
      + row('Anno precedente', fmtEur(Attr(h, ton, 'costo_anno_precedente_forno')), '#fff');
    mkOv(popShell('⚡', '56,189,248', 'Energia & Costi', 'Forno', 'fc-en-close', content), 'fc-en-close');
  }
  ...
  function openCicliPopup(c) {
    ...
    DAYS.forEach(function (d, i) {
      ...
      const cicli   = isToday ? cleanVal(S(h, c.pk_cicli_oggi))      : cleanVal(S(h, 'input_text.frarik_forno_cicli_' + d));
      const tempo   = isToday ? cleanVal(Attr(h, ton, 'Oggi'))        : cleanVal(S(h, 'input_text.frarik_forno_tempo_' + d));
      const kwRaw   = isToday ? S(h, c.pk_kwh_oggi) : S(h, 'input_number.frarik_forno_consumo_' + d);
      const costRaw = isToday ? Attr(h, ton, 'costo_oggi_forno') : S(h, 'input_number.frarik_forno_costo_' + d);
    ...
    const content = weekHtml
      ...
      + row('Ultimo ciclo', fmtEur(Attr(h, ton, 'costo_ciclo_forno')), '#7dd3fc')
      + row('Oggi', fmtEur(Attr(h, ton, 'costo_oggi_forno')), '#7dd3fc')
      + row('Questo mese', fmtEur(Attr(h, ton, 'costo_mese_forno')), '#fff')
      + row('Questo anno', fmtEur(Attr(h, ton, 'costo_anno_forno')), '#fff');
    mkOv(popShell('❄', '56,189,248', 'Cicli & Statistiche', 'Resistenza forno', 'fc-ci-close', content), 'fc-ci-close');
  }
  ...
  function openCfg(card, el) {
    ...
    const ov = mkOv(popShell('🥘', '56,189,248', 'Configura Forno', card.id || '', 'fc-cfg-close', formHtml), 'fc-cfg-close');
  ...
  function openImpostazioniHAPopup(c) {
    ...
    dToggle('input_boolean.frarik_forno_notify_push',   '📱 Push');
    dToggle('input_boolean.frarik_forno_notify_google', '🔊 Google');
    dToggle('input_boolean.frarik_forno_notify_alexa',  '🗣 Alexa');
    dTime('input_datetime.frarik_forno_orario_inizio_notifiche', '⏰ Orario inizio notifiche');
    dTime('input_datetime.frarik_forno_orario_fine_notifiche',   '⏰ Orario fine notifiche');

    dSec('🔌 Elettrodomestico');
    dToggle('input_boolean.frarik_forno_switch', 'Switch presa');
    ...
    dNum('input_number.frarik_forno_tempo_innesco_m',   'Delay spegnimento', 'min', 0, 60, 1);
    dNum('input_number.frarik_forno_avvio_ritardato_s', 'Delay riavvio',  's',   0, 300, 1);

    dSec('⏰ Spegnimento automatico');
    dToggle('automation.frarik_forno_off_automatico', 'Auto OFF abilitato');
    dTime('input_datetime.frarik_forno_off_automatico', 'Orario spegnimento');

    dSec('📝 Personalizzazione');
    dText('input_text.frarik_forno_nome',      'Nome elettrodomestico');
    dText('input_text.frarik_forno_messaggio', 'Messaggio notifica');
    dNum('input_number.costo_energia',  'Costo energia', '€/kWh', 0, 2, 0.001);
    dInfo('Ultimo reset contatori', ss('input_text.frarik_forno_data_reset'));
    ...
    const ov = mkOv(popShell('⚙', '100,116,139', 'Impostazioni', S(h, 'input_text.frarik_forno_nome') || c.name || 'Forno', closeId, swCss + rows.join('') + saveBtn + resetBtn), closeId);
  ...
  function update(card, hass, el) {
    const h = H(), c = cfgFor(card);
    const sig = [CARD.version, S(h, c.pk_power), S(h, c.pk_running), S(h, c.pk_kwh_oggi), S(h, c.pk_cicli_oggi), Attr(h, c.pk_time_on, 'Oggi'), Attr(h, c.pk_time_on, 'tempo_ciclo_forno'), Attr(h, c.pk_time_on, 'costo_oggi_forno')].join('|');
...
homeassistant:
  customize:
    package.node_anchors:
      customize: &customize
        package: 'Frarik — Centro Controllo Forno 2.0 — Frarik'
      setting:
...
####################################################
        ...
        Sensore Potenza Forno: &sensore_potenza   "{{ states('IL_TUO_SENSORE_POTENZA') | float(0) }}"
      ...
      - name: frarik_forno_time_on
        ...
        attributes:
          ...
          Mese Precedente: >
            ...
            {% endif %}
          Anno Precedente: >
            {% set hours = state_attr('sensor.frarik_forno_tempo_anno', 'last_period') | float(0) %}
            {% set minutes = ((hours % 1) * 60) | int(0) %}
            {% set hours = (hours - (hours % 1)) | int(0) %}
            {% set day = ((hours | int(0) / 24)) | int(0) %}
            {% if day | int(0) > 0 %}
            ...
            {% endif %}
          consumo_ciclo_forno: >-
            {{ (states('sensor.frarik_forno_kwh') | float(0) - states('sensor.frarik_forno_inizio_ciclo') | float(0)) | round(2) }} kWh
          costo_ciclo_forno: >-
            {{ ((states('sensor.frarik_forno_kwh') | float(0) - states('sensor.frarik_forno_inizio_ciclo') | float(0)) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_oggi_forno: >-
            {{ (states('sensor.frarik_forno_energy_oggi') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_mese_forno: >-
            {{ (states('sensor.frarik_forno_energy_mese') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_anno_forno: >-
            {{ (states('sensor.frarik_forno_energy_anno') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_ieri_forno: >-
            {{ (state_attr('sensor.frarik_forno_energy_oggi', 'last_period') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_mese_precedente_forno: >-
            {{ (state_attr('sensor.frarik_forno_energy_mese', 'last_period') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
          costo_anno_precedente_forno: >-
            {{ (state_attr('sensor.frarik_forno_energy_anno', 'last_period') | float(0) * states('input_number.costo_energia') | float(0)) | round(2) }}
...
input_datetime:
  ...
  frarik_forno_orario_inizio_notifiche:
    name: Orario Inizio Notifiche Forno
    has_date: false
    has_time: true

  frarik_forno_orario_fine_notifiche:
    name: Orario Fine Notifiche Forno
    has_date: false
    has_time: true

  frarik_forno_off_automatico:
    name: Forno Off Automatico
    has_date: false
    has_time: true
...
- alias: frarik_forno_automazione
  id: frarik_forno_automazione
  max_exceeded: silent
  trigger:
    ...
    - conditions:
      ...
      - service: input_text.set_value
        target:
          entity_id: >
            ...
            {{ {'Monday':'input_text.frarik_forno_cicli_lunedi','Tuesday':'input_text.frarik_forno_cicli_martedi','Wednesday':'input_text.frarik_forno_cicli_mercoledi','Thursday':'input_text.frarik_forno_cicli_giovedi','Friday':'input_text.frarik_forno_cicli_venerdi','Saturday':'input_text.frarik_forno_cicli_sabato','Sunday':'input_text.frarik_forno_cicli_domenica'}[g] }}
        data:
      ...
      - service: input_text.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_text.frarik_forno_tempo_lunedi','Tuesday':'input_text.frarik_forno_tempo_martedi','Wednesday':'input_text.frarik_forno_tempo_mercoledi','Thursday':'input_text.frarik_forno_tempo_giovedi','Friday':'input_text.frarik_forno_tempo_venerdi','Saturday':'input_text.frarik_forno_tempo_sabato','Sunday':'input_text.frarik_forno_tempo_domenica'}[g] }}
        data:
      ...
      - service: input_number.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_number.frarik_forno_consumo_lunedi','Tuesday':'input_number.frarik_forno_consumo_martedi','Wednesday':'input_number.frarik_forno_consumo_mercoledi','Thursday':'input_number.frarik_forno_consumo_giovedi','Friday':'input_number.frarik_forno_consumo_venerdi','Saturday':'input_number.frarik_forno_consumo_sabato','Sunday':'input_number.frarik_forno_consumo_domenica'}[g] }}
        data:
      ...
      - service: input_number.set_value
        target:
          entity_id: >
            {% set g = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][now().weekday()] %}
            {{ {'Monday':'input_number.frarik_forno_costo_lunedi','Tuesday':'input_number.frarik_forno_costo_martedi','Wednesday':'input_number.frarik_forno_costo_mercoledi','Thursday':'input_number.frarik_forno_costo_giovedi','Friday':'input_number.frarik_forno_costo_venerdi','Saturday':'input_number.frarik_forno_costo_sabato','Sunday':'input_number.frarik_forno_costo_domenica'}[g] }}
        data:
    ...
    - choose:
      - conditions:
        - condition: trigger
          id: fine_ciclo
        - condition: time
          after: 'input_datetime.frarik_forno_orario_inizio_notifiche'
          before: 'input_datetime.frarik_forno_orario_fine_notifiche'
        - condition: state
        ...
        - service: tts.google_translate_say
          ...
          data:
            entity_id: *google
            message: >-
              {%- set dur=states('input_text.frarik_forno_ultimo_ciclo')|trim -%}
              ...
              {{- states('input_text.frarik_forno_messaggio') }} in {{ hv+sep+mv -}}
      ...
      - conditions:
        ...
        - repeat:
            ...
            - service: "notify.{{ repeat.item.service }}"
              continue_on_error: true
              data:
                title: "{{ states('input_text.frarik_forno_nome') }}"
                message: >-
                  🫧 {{ states('input_text.frarik_forno_messaggio') }}
...
- alias: frarik_forno_off_automatico
  ...
  trigger:
    - platform: time
      at: 'input_datetime.frarik_forno_off_automatico'
      id: auto_off
  condition: []
  action:
    - choose:
...
`;
  ...
  function _openWizard(hass, onDone, _tpl, opts) {
    ...
    var saved = null;
    try { saved = JSON.parse(localStorage.getItem(_FRN_WIZ_KEY) || 'null'); } catch(e) {}
    ...
    function renderWiz() {
      sr.innerHTML = '<style>'
        ...
        + '.wd-panel{width:100%;max-height:88vh;display:flex;flex-direction:column;background:#080f18;border:1px solid rgba(56,189,248,.3);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -12px 60px rgba(0,0,0,.8);color:#fff;overflow:hidden;animation:frnUp .22s cubic-bezier(.32,1.12,.56,1)}'
        + '@keyframes frnUp{from{transform:translateY(100%)}to{transform:translateY(0)}}'
        ...
        + '<div><div class="wd-tit">' + (isUpdate ? 'Aggiorna PKG Forno' : 'Installa PKG Forno') + '</div><div class="wd-sub">frarik_forno.yaml → config/packages/</div></div>'
        ...
        + '<div class="wd-frow"><input class="wd-inp" id="f-potenza" type="text" autocomplete="off" placeholder="sensor.presa_forno_potenza" value="' + ((saved && saved.potenza) || '').replace(/"/g, '&quot;') + '"><div class="wd-drop" id="d-potenza"></div></div>'
        + '<div class="wd-lbl">Switch Presa Forno</div>'
        + '<div class="wd-frow"><input class="wd-inp" id="f-switch" type="text" autocomplete="off" placeholder="switch.presa_forno" value="' + ((saved && saved.sw) || '').replace(/"/g, '&quot;') + '"><div class="wd-drop" id="d-switch"></div></div>'
        + '</div>'
      ...
      sr.getElementById('wd-install').addEventListener('click', function() {
        ...
        fetch(base + '/api/frarik/pkg/install', {
          ...
          body: JSON.stringify({name: 'frarik/frarik_forno.yaml', content: yaml})
        }).then(function(r) { return r.json().then(function(j) { return {r: r, j: j}; }); })
          .then(function(res) {
            destroy();
            if (res.r.ok && res.j.ok) {
              try { if (typeof window.showToast === 'function') window.showToast('📦 PKG Forno installato! Riavvia HA.'); } catch(e) {}
  ...
  try { console.log('[FratechStore] Card registrata: forno v' + CARD.version); } catch (e) {}
})();
