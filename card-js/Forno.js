...
(function () {
  ...
  function render(card) {
    ...
    return css
      ...
      + '<div class="fc-hdr-tit">' + (c.name || 'Forno') + '</div>'
  ...
  function openEnergiaPopup(c) {
    ...
    const content = '<div style="background:rgba(56,189,248,.1);border-radius:12px;padding:12px 14px;text-align:center;margin-bottom:12px">'
      ...
      + row('Mese precedente', fmtEur(Attr(h, ton, 'costo_mese_prec_forno')), '#fff')
      + row('Questo anno', fmtEur(Attr(h, ton, 'costo_anno_forno')), '#7dd3fc')
      + row('Anno precedente', fmtEur(Attr(h, ton, 'costo_anno_prec_forno')), '#fff');
  ...
  function openCicliPopup(c) {
    ...
    DAYS.forEach(function (d, i) {
      ...
      const cicli = isToday ? cleanVal(S(h, c.pk_cicli_oggi))      : cleanVal(S(h, 'input_text.' + d + '_forno_cicli'));
      const tempo = isToday ? cleanVal(Attr(h, ton, 'Oggi'))        : cleanVal(S(h, 'input_text.' + d + '_forno_tempo'));
  ...
  function openImpostazioniHAPopup(c) {
    ...
    dTime('input_datetime.frarik_forno_notifiche_inizio', '⏰ Orario inizio notifiche');
    dTime('input_datetime.frarik_forno_notifiche_fine',   '⏰ Orario fine notifiche');
    ...
    dTime('input_datetime.frarik_forno_off', 'Orario spegnimento');
    ...
    const ov = mkOv(popShell('⚙', '100,116,139', 'Impostazioni', c.name || 'Forno', closeId, swCss + rows.join('') + saveBtn + resetBtn), closeId);
