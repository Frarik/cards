...
(function () {
  ...
  function Attr(h, id, attr) { const s = h && id && h.states && h.states[id]; return s && s.attributes ? s.attributes[attr] : null; }
  ...
  function fmtEur(v) { if (v == null) return '—'; const n = parseFloat(v); return isNaN(n) ? '—' : n.toFixed(2) + ' €'; }
  function fmtKwh(v) { if (v == null) return '—'; const n = parseFloat(v); return isNaN(n) ? '—' : n.toFixed(3) + ' kWh'; }
  ...
  function render(card) {
    ...
    // PKG fixed sensors
    const pwV     = num(S(h, 'sensor.sensore_potenza_server_w'));
    const enOggi  = S(h, 'sensor.energia_oggi_server');
    const enMese  = S(h, 'sensor.energia_mese_server');
    const enAnno  = S(h, 'sensor.energia_anno_server');
    const coOggi  = S(h, 'sensor.costo_consumo_giornaliero_server');
    const coMese  = S(h, 'sensor.costo_consumo_mensile_server');
    const coAnno  = S(h, 'sensor.costo_consumo_annuale_server');
    const haUptime   = S(h, 'sensor.template_tempo_di_avvio_homeassistant');
    const entCount   = S(h, 'sensor.conteggio_entita');
    const lastBackup = S(h, 'sensor.ultimo_backup_google_drive');
    const coreOk  = S(h, 'sensor.update_core_card') === 'Aggiornato';
    const supOk   = S(h, 'sensor.update_supervisor_card') === 'Aggiornato';
    const addonOk = S(h, 'sensor.supervisor_update_addon_card') === 'Aggiornati';
    const hacsOk  = S(h, 'sensor.hacs_store_card') === 'Aggiornato';
    ...
    const css = '<style>'
      ...
      + '#' + rid + ' [data-sya="popup-energia"]{cursor:pointer;transition:filter .12s;}'
      + '#' + rid + ' [data-sya="popup-energia"]:hover{filter:brightness(1.15);}'
      + '#' + rid + ' [data-sya="popup-agg"]{cursor:pointer;transition:filter .12s;}'
      + '#' + rid + ' [data-sya="popup-agg"]:hover{filter:brightness(1.15);}'
      + '#' + rid + ' [data-sya="stat"]{cursor:pointer;transition:filter .12s,transform .1s;}'
      + '#' + rid + ' [data-sya="stat"]:hover{filter:brightness(1.2);}'
      + '#' + rid + ' [data-sya="stat"]:active{transform:scale(.95);}'
      + '</style>';
    ...
    const tempSection = '<div style="display:flex;align-items:stretch;gap:8px;position:relative;z-index:1">'
      + '<div data-sya="stat" data-eid="sensor.cpu_temperatura" data-lbl="Temperatura CPU" style="display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(255,255,255,.05);border-radius:12px;padding:7px 12px;flex-shrink:0;min-width:72px;gap:3px">'
    ...
    const energiaSection = '<div data-sya="popup-energia" style="background:rgba(249,115,22,.07);border:1px solid rgba(249,115,22,.2);border-radius:11px;padding:7px 10px;position:relative;z-index:1">'
      ...
      + '<div style="font-size:9px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em">⚡ Energia</div>'
      + '<div style="font-size:9px;color:rgba(255,255,255,.25)">dettagli ›</div>'
      + '</div>'
    ...
    const haInfoSection = '<div style="display:flex;gap:6px;position:relative;z-index:1">'
      + '<div data-sya="stat" data-eid="sensor.template_tempo_di_avvio_homeassistant" data-lbl="HA Uptime" style="flex:1;background:rgba(167,139,250,.07);border:1px solid rgba(167,139,250,.15);border-radius:10px;padding:6px 9px;cursor:pointer">'
      ...
      + '<div data-syv="ha-uptime" style="font-size:11px;font-weight:800;color:#a78bfa">' + (haUptime || '—') + '</div>'
      + '</div>'
      ...
      + '<div data-syv="ent-count" style="font-size:11px;font-weight:800;color:#38bdf8">' + (entCount || '—') + '</div>'
      + '</div>'
      + '<div style="flex:1.6;background:rgba(34,197,94,.06);border:1px solid rgba(34,197,94,.15);border-radius:10px;padding:6px 9px">'
      ...
      + '<div data-syv="last-bk" style="font-size:9px;font-weight:700;color:#86efac;line-height:1.3">' + (lastBackup || '—') + '</div>'
      + '</div>'
      + '</div>';
    ...
    const aggSection = '<div data-sya="popup-agg" style="background:' + (anyUpd ? 'rgba(249,115,22,.07)' : 'rgba(34,197,94,.05)') + ';border:1px solid ' + (anyUpd ? 'rgba(249,115,22,.2)' : 'rgba(34,197,94,.15)') + ';border-radius:11px;padding:7px 10px;position:relative;z-index:1">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px">'
      + '<div style="font-size:9px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em">🔄 Aggiornamenti</div>'
      + '<div style="font-size:9px;color:rgba(255,255,255,.25)">dettagli ›</div>'
      + '</div>'
      + '<div data-syv="agg-badges" style="display:flex;gap:5px">'
      + updBadge('Core', coreOk)
      + updBadge('Supv', supOk)
      + updBadge('Addon', addonOk)
      + updBadge('HACS', hacsOk)
      + '</div>'
      + '</div>';
    ...
    const togglesSection = '<div style="background:rgba(139,92,246,.06);border:1px solid rgba(139,92,246,.18);border-radius:11px;padding:7px 10px;position:relative;z-index:1">'
      + '<div style="font-size:9px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">⚙ Automazioni</div>'
      ...
      + togBtn('input_boolean.on_off_ventola_rack', 'Ventola', ventolOn)
      + '</div>'
      + '</div>';
  ...
  function _patch(card, el) {
    ...
    function sv(k, t) { const e = el.querySelector('[data-syv="' + k + '"]'); if (e && t != null) e.textContent = t; }
    ...
    if (bdgRow) {
      const bdgs = statusBadges(cpuV, ramV, tmpV, l1, swpV);
      bdgRow.innerHTML = bdgs.map(b => '<div style="font-size:10px;font-weight:700;padding:3px 9px;border-radius:99px;background:' + b[1] + '18;border:1px solid ' + b[1] + '44;color:' + b[1] + '">' + b[0] + '</div>').join('');
    }
    ...
    if (togRow) {
      togRow.innerHTML = togBtn('input_boolean.on_off_alert_ha', 'Alert', isOn(h, 'input_boolean.on_off_alert_ha'))
        ...
        + togBtn('input_boolean.on_off_ventola_rack', 'Ventola', isOn(h, 'input_boolean.on_off_ventola_rack'));
    }
  }

  /* ── Popup energia ── */
  function openEnergiaPopup() {
    const h = H();
    function row(label, val, col) {
      return '<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.05)">'
        ...
        + '<span style="font-size:13px;font-weight:800;color:' + (col || '#fbbf24') + '">' + val + '</span>'
        + '</div>';
    }
    ...
    ov.innerHTML = '<style>@keyframes syEUp{from{transform:translateY(100%)}to{transform:translateY(0)}}.en-content{overflow-y:auto;scrollbar-width:none;-ms-overflow-style:none}.en-content::-webkit-scrollbar{display:none}</style>'
      + '<div style="width:100%;max-height:72vh;display:flex;flex-direction:column;background:#0a0816;border:1px solid rgba(249,115,22,.25);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -12px 60px rgba(0,0,0,.7);animation:syEUp .22s cubic-bezier(.32,1.12,.56,1);overflow:hidden">'
        + '<div style="display:flex;align-items:center;gap:10px;padding:14px 16px 12px;border-bottom:1px solid rgba(255,255,255,.07);flex-shrink:0">'
          + '<div style="width:34px;height:34px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:16px;background:rgba(249,115,22,.15);border:1px solid rgba(249,115,22,.3);color:#fb923c;flex-shrink:0">⚡</div>'
          + '<div><div style="font-size:14px;font-weight:800;color:#fff">Energia & Costi</div><div style="font-size:11px;color:rgba(255,255,255,.45);margin-top:1px">Consumo server</div></div>'
          + '<button id="en-close" style="margin-left:auto;width:28px;height:28px;border-radius:8px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px;color:rgba(255,255,255,.5);background:rgba(255,255,255,.07);border:none">✕</button>'
        + '</div>'
        + '<div class="en-content" style="padding:14px 16px;display:flex;flex-direction:column;gap:12px">'
          + '<div style="background:rgba(249,115,22,.1);border-radius:12px;padding:12px 14px;text-align:center">'
            + '<div style="font-size:10px;font-weight:700;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Potenza Attuale</div>'
            + '<div style="font-size:28px;font-weight:900;color:#fb923c">' + (num(S(h, 'sensor.sensore_potenza_server_w')) == null ? '—' : num(S(h, 'sensor.sensore_potenza_server_w')).toFixed(0) + ' W') + '</div>'
          + '</div>'
          + '<div>'
            + '<div style="font-size:10px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Consumo Energia</div>'
            + row('Oggi', fmtKwh(S(h, 'sensor.energia_oggi_server')), '#fdba74')
            + row('Questo mese', fmtKwh(S(h, 'sensor.energia_mese_server')), '#fdba74')
            + row('Questo anno', fmtKwh(S(h, 'sensor.energia_anno_server')), '#fdba74')
          + '</div>'
          + '<div>'
            + '<div style="font-size:10px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Costi (€)</div>'
            + row('Oggi', fmtEur(S(h, 'sensor.costo_consumo_giornaliero_server')), '#fbbf24')
            + row('Ieri', fmtEur(S(h, 'sensor.costo_consumo_ieri_server')), 'rgba(255,255,255,.5)')
            + row('Questo mese', fmtEur(S(h, 'sensor.costo_consumo_mensile_server')), '#fbbf24')
            + row('Mese precedente', fmtEur(S(h, 'sensor.costo_consumo_mese_precedente_server')), 'rgba(255,255,255,.5)')
            + row('Questo anno', fmtEur(S(h, 'sensor.costo_consumo_annuale_server')), '#fbbf24')
            + row('Anno precedente', fmtEur(S(h, 'sensor.costo_consumo_anno_precedente_server')), 'rgba(255,255,255,.5)')
          + '</div>'
        + '</div>'
      + '</div>';
    document.body.appendChild(ov);
    const close = () => { try { document.body.removeChild(ov); } catch (e) {} };
    ov.querySelector('#en-close').addEventListener('click', close);
    ov.addEventListener('click', e => { if (e.target === ov) close(); });
  }

  /* ── Popup aggiornamenti ── */
  function openAggPopup() {
    const h = H();
    function badge(label, isOk, text) {
      ...
      return '<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;border-radius:10px;background:' + col + '10;border:1px solid ' + col + '25;margin-bottom:6px">'
        ...
        + '<span style="font-size:11px;font-weight:800;color:' + col + '">' + (isOk ? '✓ Aggiornato' : '! ' + (text || 'Da aggiornare')) + '</span>'
        + '</div>';
    }
    const coreS  = S(h, 'sensor.update_core_card');
    const supS   = S(h, 'sensor.update_supervisor_card');
    const addonS = S(h, 'sensor.supervisor_update_addon_card');
    const hacsS  = S(h, 'sensor.hacs_store_card');
    const hacsN  = S(h, 'sensor.hacs');

    const ov = document.createElement('div');
    ov.style.cssText = 'position:fixed;inset:0;z-index:100000;display:flex;align-items:flex-end;background:rgba(0,0,0,.6);backdrop-filter:blur(4px)';
    ov.innerHTML = '<style>@keyframes syAUp{from{transform:translateY(100%)}to{transform:translateY(0)}}.ag-content{overflow-y:auto;scrollbar-width:none;-ms-overflow-style:none}.ag-content::-webkit-scrollbar{display:none}</style>'
      + '<div style="width:100%;max-height:72vh;display:flex;flex-direction:column;background:#0a0816;border:1px solid rgba(56,189,248,.25);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -12px 60px rgba(0,0,0,.7);animation:syAUp .22s cubic-bezier(.32,1.12,.56,1);overflow:hidden">'
        + '<div style="display:flex;align-items:center;gap:10px;padding:14px 16px 12px;border-bottom:1px solid rgba(255,255,255,.07);flex-shrink:0">'
          + '<div style="width:34px;height:34px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:16px;background:rgba(56,189,248,.15);border:1px solid rgba(56,189,248,.3);color:#38bdf8;flex-shrink:0">🔄</div>'
          + '<div><div style="font-size:14px;font-weight:800;color:#fff">Aggiornamenti</div><div style="font-size:11px;color:rgba(255,255,255,.45);margin-top:1px">Stato sistema HA</div></div>'
          + '<button id="ag-close" style="margin-left:auto;width:28px;height:28px;border-radius:8px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px;color:rgba(255,255,255,.5);background:rgba(255,255,255,.07);border:none">✕</button>'
        + '</div>'
        + '<div class="ag-content" style="padding:14px 16px">'
          + badge('Core HA', coreS === 'Aggiornato', coreS)
          + badge('Supervisor', supS === 'Aggiornato', supS)
          + badge('Add-on', addonS === 'Aggiornati', addonS)
          + badge('HACS', hacsS === 'Aggiornato', hacsS)
          + (hacsN && parseInt(hacsN) > 0 ? '<div style="margin-top:8px;padding:8px 12px;border-radius:10px;background:rgba(249,115,22,.1);border:1px solid rgba(249,115,22,.2);font-size:11px;color:#fdba74">' + hacsN + ' aggiornamenti HACS disponibili</div>' : '')
        + '</div>'
      + '</div>';
    document.body.appendChild(ov);
    const close = () => { try { document.body.removeChild(ov); } catch (e) {} };
    ov.querySelector('#ag-close').addEventListener('click', close);
    ov.addEventListener('click', e => { if (e.target === ov) close(); });
  }

  /* ── Popup grafico storico ── */
  function openHistPopup(entityId, label) {
    const ov = document.createElement('div');
    ov.style.cssText = 'position:fixed;inset:0;z-index:100000;display:flex;align-items:flex-end;background:rgba(0,0,0,.6);backdrop-filter:blur(4px)';
    ov.innerHTML = '<style>@keyframes syHUp{from{transform:translateY(100%)}to{transform:translateY(0)}}.sh-content{overflow-y:auto;scrollbar-width:none;-ms-overflow-style:none}.sh-content::-webkit-scrollbar{display:none}</style>'
      + '<div style="width:100%;max-height:72vh;display:flex;flex-direction:column;background:#0a0816;border:1px solid rgba(251,191,36,.25);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -12px 60px rgba(0,0,0,.7);animation:syHUp .22s cubic-bezier(.32,1.12,.56,1);overflow:hidden">'
        + '<div style="display:flex;align-items:center;gap:10px;padding:14px 16px 12px;border-bottom:1px solid rgba(255,255,255,.07);flex-shrink:0">'
          ...
          + '<div><div style="font-size:14px;font-weight:800;color:#fff">' + label + '</div><div style="font-size:11px;color:rgba(255,255,255,.45);margin-top:1px">Ultime 24 ore</div></div>'
          + '<button id="sh-close" style="margin-left:auto;width:28px;height:28px;border-radius:8px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px;color:rgba(255,255,255,.5);background:rgba(255,255,255,.07);border:none">✕</button>'
        + '</div>'
        + '<div class="sh-content" style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">'
          + '<div id="sh-stats" style="display:flex;gap:8px"></div>'
          + '<div id="sh-chart" style="min-height:120px;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,.3);font-size:12px">Caricamento dati…</div>'
        + '</div>'
      + '</div>';
    document.body.appendChild(ov);
    const close = () => { try { document.body.removeChild(ov); } catch (e) {} };
    ...
    callApi('GET', 'history/period/' + start.toISOString() + '?filter_entity_id=' + entityId + '&end_time=' + now.toISOString()).then(function (data) {
      const chartEl = ov.querySelector('#sh-chart');
      const statsEl = ov.querySelector('#sh-stats');
      if (!chartEl) return;
      try {
        ...
        if (statsEl) {
          function stat(l, v, col) { return '<div style="flex:1;background:rgba(255,255,255,.05);border-radius:10px;padding:8px 10px;text-align:center"><div style="font-size:9px;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px">' + l + '</div><div style="font-size:14px;font-weight:800;color:' + col + '">' + v.toFixed(1) + '</div></div>'; }
          statsEl.innerHTML = stat('Min', minV, '#38bdf8') + stat('Attuale', cur, '#fbbf24') + stat('Max', maxV, '#ef4444');
        }
  ...
  function mount(card, hass, el) {
    ...
    el.addEventListener('click', function (e) {
      ...
      if (togEl) {
        const eid = togEl.dataset.eid;
        const h = H(), cur = h && h.states && h.states[eid] && h.states[eid].state;
        callSvc('homeassistant', cur === 'on' ? 'turn_off' : 'turn_on', { entity_id: eid });
        return;
      }
      if (e.target.closest('[data-sya="popup-energia"]')) { openEnergiaPopup(); return; }
      if (e.target.closest('[data-sya="popup-agg"]')) { openAggPopup(); return; }
      const statEl = e.target.closest('[data-sya="stat"]');
      if (statEl) { openHistPopup(statEl.dataset.eid, statEl.dataset.lbl || statEl.dataset.eid); return; }
    });
  }
  ...
  function openCfg(card, el) {
    ...
    ov.querySelector('#sy-save').addEventListener('click', function () {
      ...
      var wV = parseInt(ov.querySelector('#sy-cardw').value) || 100;
      save(card, {
        name: g('sy-name'), cpu: g('sy-cpu'), ram: g('sy-ram'), disk: g('sy-disk'), swap: g('sy-swap'),
  ...
  var CARD = {
    id: 'system-card', name: 'Mini-PC', icon: '🖥️', version: '4.6',
    desc: 'Mini-PC / Server: ring CPU/RAM/Disco/Swap, energia & costi, aggiornamenti HA, toggle automazioni, grafici storici.',
    colSpan: 2, rowSpan: 4,
