...
(function () {
  ...
  function autodetect(h) {
    ...
    return {
      ...
      temp: f('sensor.processor_temperature', 'sensor.system_monitor_processor_temperature', 'sensor.cpu_temperature', 'sensor.cpu_temperatura'),
  ...
  function cfgFor(card) {
    const c = load(card), a = autodetect(H()), pk = pkDefaults();
    const r = {};
    Object.keys(a).forEach(k => { r[k] = (c[k] !== undefined && c[k] !== '') ? c[k] : a[k]; });
    Object.keys(pk).forEach(k => { r[k] = (c[k] !== undefined && c[k] !== '') ? c[k] : pk[k]; });
    return r;
  }
  ...
  function togBtn(id, label, on) {
    return '<button data-sya="toggle" data-eid="' + id + '" style="padding:4px 10px;border-radius:99px;border:1px solid '
      + (on ? 'rgba(34,197,94,.5)' : 'rgba(255,255,255,.15)') + ';background:'
      + (on ? 'rgba(34,197,94,.12)' : 'rgba(255,255,255,.05)') + ';color:'
      + (on ? '#86efac' : 'rgba(255,255,255,.4)') + ';font-size:10px;font-weight:700;cursor:pointer;white-space:nowrap;transition:all .2s">' + label + '</button>';
  }

  function updBadge(label, isOk) {
    const col = isOk ? '#22c55e' : '#f97316';
    return '<div style="display:flex;flex-direction:column;align-items:center;gap:2px;padding:5px 6px;border-radius:8px;background:' + col + '12;border:1px solid ' + col + '30;flex:1;min-width:44px">'
      + '<div style="font-size:8px;font-weight:700;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.05em">' + label + '</div>'
      + '<div style="font-size:11px;font-weight:900;color:' + col + '">' + (isOk ? '✓' : '!') + '</div>'
      + '</div>';
  }
  ...
  function ekgSVG(data, w, h, col, gid) {
    ...
    return '<svg width="' + w + '" height="' + h + '" viewBox="0 0 ' + w + ' ' + h + '" style="display:block;overflow:visible">'
      + grid + '<defs><linearGradient id="' + gid + '" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="' + col + '" stop-opacity=".35"/><stop offset="100%" stop-color="' + col + '" stop-opacity="0"/></linearGradient></defs>'
  ...
  function dualNetSVG(dataIn, dataOut, w, h, gidI, gidO) {
    ...
    return '<svg width="' + w + '" height="' + h + '" viewBox="0 0 ' + w + ' ' + h + '" style="display:block;overflow:hidden">'
      + '<defs><linearGradient id="' + gidI + '" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#38bdf8" stop-opacity=".3"/><stop offset="100%" stop-color="#38bdf8" stop-opacity="0"/></linearGradient>'
      + '<linearGradient id="' + gidO + '" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#a78bfa" stop-opacity=".25"/><stop offset="100%" stop-color="#a78bfa" stop-opacity="0"/></linearGradient></defs>'
      + (pi ? '<polygon points="' + pi + ' ' + w + ',' + h + ' 0,' + h + '" fill="url(#' + gidI + ')"/><polyline points="' + pi + '" fill="none" stroke="#38bdf8" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>' : '')
      + (po ? '<polygon points="' + po + ' ' + w + ',' + h + ' 0,' + h + '" fill="url(#' + gidO + ')"/><polyline points="' + po + '" fill="none" stroke="#a78bfa" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>' : '')
      + '</svg>';
  }
  ...
  function histSVG(pts, minV, maxV, w, h, col, gid) {
    ...
    const coords = pts.map((v, i) => ((i / (pts.length - 1)) * w).toFixed(1) + ',' + (h - 4 - ((v - minV) / range) * (h - 8)).toFixed(1));
    return '<svg width="100%" height="' + h + '" viewBox="0 0 ' + w + ' ' + h + '" preserveAspectRatio="none" style="display:block">'
      + '<defs><linearGradient id="' + gid + '" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="' + col + '" stop-opacity=".4"/><stop offset="100%" stop-color="' + col + '" stop-opacity="0"/></linearGradient></defs>'
      + '<polygon points="' + coords.join(' ') + ' ' + w + ',' + h + ' 0,' + h + '" fill="url(#' + gid + ')"/>'
      + '<polyline points="' + coords.join(' ') + '" fill="none" stroke="' + col + '" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>'
      + '</svg>';
  }
  ...
  function ringHTML(key, pct, col, label, sz, sub) {
    ...
    return '<div style="display:flex;flex-direction:column;align-items:center;gap:2px">'
      ...
      + '<circle data-arc="' + key + '" cx="' + cx + '" cy="' + cy + '" r="' + r + '" fill="none" stroke="' + col + '" stroke-width="6" stroke-dasharray="' + dash + ' ' + circ + '" stroke-linecap="round" transform="rotate(-90 ' + cx + ' ' + cy + ')" style="transition:stroke-dasharray .9s ease-in-out,stroke .5s;filter:drop-shadow(0 0 7px ' + col + '99)"/>'
      + '<text data-txt="' + key + '" x="' + cx + '" y="' + (cy + 1) + '" text-anchor="middle" dominant-baseline="middle" fill="' + col + '" font-size="' + (s * 0.185).toFixed(0) + 'px" font-weight="800" font-family="system-ui,sans-serif">' + (pct == null ? '—' : Math.round(pct) + '%') + '</text>'
      + '</svg>'
  ...
  function render(card) {
    ...
    const ramTot = num(S(h, c.pk_ram_tot));
    const dskTot = num(S(h, c.pk_disk_tot));
    ...
    const enOggi   = S(h, c.pk_en_oggi);
    const coOggi   = S(h, c.pk_co_oggi);
    const coMese   = S(h, c.pk_co_mese);
    const haUptime = S(h, c.pk_ha_uptime);
    const srvUptime= S(h, c.pk_srv_uptime);
    const entCount = S(h, c.pk_entita);
    const lastBk   = S(h, c.pk_backup);
    const coreOk   = S(h, c.pk_core) === 'Aggiornato';
    const supOk    = S(h, c.pk_sup) === 'Aggiornato';
    const addonOk  = S(h, c.pk_addon) === 'Aggiornati';
    const hacsOk   = S(h, c.pk_hacs_card) === 'Aggiornato';
    const hacsN    = num(S(h, c.pk_hacs));
    const certS    = c.pk_cert ? S(h, c.pk_cert) : null;
    const anyUpd   = !coreOk || !supOk || !addonOk || !hacsOk;
    const alertOn  = isOn(h, 'input_boolean.on_off_alert_ha');
    const backupOn = isOn(h, 'input_boolean.ha_backup');
    const reportOn = isOn(h, 'input_boolean.ha_report');
    const riavHaOn = isOn(h, 'input_boolean.on_off_riavvio_ha');
    const riavSrvOn= isOn(h, 'input_boolean.on_off_riavvio_server');
    const ventolOn = isOn(h, 'input_boolean.on_off_ventola_rack');
    ...
    function loadRow(lbl, v, k) {
      ...
      return '<div style="display:flex;align-items:center;gap:5px">'
        ...
        + '<div style="flex:1;height:5px;border-radius:99px;background:rgba(255,255,255,.07);overflow:hidden"><div data-bar="' + k + '" style="height:100%;width:' + p.toFixed(1) + '%;background:' + col + ';border-radius:99px;transition:width .9s ease-in-out,background .5s"></div></div>'
    ...
    const badgesRow = '<div data-syv="badges" style="display:flex;gap:5px;flex-wrap:wrap;position:relative;z-index:1">'
      + bdgs.map(b => '<div style="font-size:10px;font-weight:700;padding:3px 9px;border-radius:99px;background:' + b[1] + '18;border:1px solid ' + b[1] + '44;color:' + b[1] + '">' + b[0] + '</div>').join('')
      + '</div>';

    const rings = '<div style="display:flex;justify-content:space-around;align-items:flex-start;position:relative;z-index:1">'
      + ringHTML('cpu', cpuV, cpuC, 'CPU', 70, '') + ringHTML('ram', ramV, ramC, 'RAM', 70, ramSub)
      + ringHTML('dsk', dskV, dskC, 'Disco', 70, dskSub) + ringHTML('swp', swpV, swpC, 'Swap', 70, swpSub)
      + '</div>';
    ...
    const netSection = (c.netin || c.netout) ? ('<div style="background:rgba(56,189,248,.06);border:1px solid rgba(56,189,248,.15);border-radius:11px;padding:6px 10px;position:relative;z-index:1">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px"><div style="font-size:9px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em">🌐 Rete</div>'
      + '<div style="display:flex;gap:10px"><div data-syv="ni" style="font-size:10px;font-weight:700;color:#38bdf8">↓ ' + fmtNet(niV) + '</div><div data-syv="no" style="font-size:10px;font-weight:700;color:#a78bfa">↑ ' + fmtNet(noV) + '</div></div></div>'
      + '<div data-syv="net-sp" style="height:32px"></div></div>') : '';
    ...
    // ── PKG sections ──
    const energiaSection = '<div data-sya="popup-energia" style="background:rgba(249,115,22,.07);border:1px solid rgba(249,115,22,.2);border-radius:11px;padding:7px 10px;position:relative;z-index:1">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px"><div style="font-size:9px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em">⚡ Energia</div><div style="font-size:9px;color:rgba(255,255,255,.25)">dettagli ›</div></div>'
      + '<div style="display:flex;gap:6px;flex-wrap:wrap">'
      + '<div style="display:flex;flex-direction:column;gap:1px;flex:1;min-width:54px"><div style="font-size:8px;color:rgba(255,255,255,.4)">Potenza</div><div data-syv="pw" style="font-size:14px;font-weight:800;color:#fb923c">' + (pwV == null ? '—' : pwV.toFixed(0) + ' W') + '</div></div>'
      + '<div style="display:flex;flex-direction:column;gap:1px;flex:1;min-width:54px"><div style="font-size:8px;color:rgba(255,255,255,.4)">Oggi</div><div data-syv="en-oggi" style="font-size:11px;font-weight:800;color:#fdba74">' + fmtKwh(enOggi) + '</div></div>'
      + '<div style="display:flex;flex-direction:column;gap:1px;flex:1;min-width:54px"><div style="font-size:8px;color:rgba(255,255,255,.4)">Costo oggi</div><div data-syv="co-oggi" style="font-size:11px;font-weight:800;color:#fbbf24">' + fmtEur(coOggi) + '</div></div>'
      + '<div style="display:flex;flex-direction:column;gap:1px;flex:1;min-width:54px"><div style="font-size:8px;color:rgba(255,255,255,.4)">Mese</div><div data-syv="co-mese" style="font-size:11px;font-weight:800;color:#fbbf24">' + fmtEur(coMese) + '</div></div>'
      + '</div></div>';

    const haInfoSection = '<div style="display:flex;gap:6px;position:relative;z-index:1">'
      + '<div data-sya="stat" data-eid="' + (c.pk_ha_uptime || '') + '" data-lbl="Uptime HA" style="flex:1;background:rgba(167,139,250,.07);border:1px solid rgba(167,139,250,.15);border-radius:10px;padding:6px 9px">'
      + '<div style="font-size:8px;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em;margin-bottom:2px">HA attivo da</div>'
      + '<div data-syv="ha-uptime" style="font-size:11px;font-weight:800;color:#a78bfa">' + (haUptime || '—') + '</div></div>'
      + '<div data-sya="stat" data-eid="' + (c.pk_srv_uptime || '') + '" data-lbl="Uptime Server" style="flex:1;background:rgba(99,102,241,.07);border:1px solid rgba(99,102,241,.15);border-radius:10px;padding:6px 9px">'
      + '<div style="font-size:8px;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em;margin-bottom:2px">Server attivo da</div>'
      + '<div data-syv="srv-uptime" style="font-size:11px;font-weight:800;color:#818cf8">' + (srvUptime ? uptimeText(h, c.pk_srv_uptime) : '—') + '</div></div>'
      + '<div style="flex:1;background:rgba(56,189,248,.07);border:1px solid rgba(56,189,248,.15);border-radius:10px;padding:6px 9px">'
      + '<div style="font-size:8px;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em;margin-bottom:2px">Entità totali</div>'
      + '<div data-syv="ent-count" style="font-size:11px;font-weight:800;color:#38bdf8">' + (entCount || '—') + '</div></div>'
      + '<div style="flex:1.4;background:rgba(34,197,94,.06);border:1px solid rgba(34,197,94,.15);border-radius:10px;padding:6px 9px">'
      + '<div style="font-size:8px;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em;margin-bottom:2px">Ultimo backup</div>'
      + '<div data-syv="last-bk" style="font-size:9px;font-weight:700;color:#86efac;line-height:1.3">' + (lastBk || '—') + '</div></div>'
      + '</div>';

    const certCol = certS == null ? 'rgba(255,255,255,.3)' : (certS.toLowerCase().includes('scad') || certS === 'off' ? '#ef4444' : '#22c55e');
    const aggSection = '<div data-sya="popup-agg" style="background:' + (anyUpd ? 'rgba(249,115,22,.07)' : 'rgba(34,197,94,.05)') + ';border:1px solid ' + (anyUpd ? 'rgba(249,115,22,.2)' : 'rgba(34,197,94,.15)') + ';border-radius:11px;padding:7px 10px;position:relative;z-index:1">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px"><div style="font-size:9px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em">🔄 Aggiornamenti</div><div style="font-size:9px;color:rgba(255,255,255,.25)">dettagli ›</div></div>'
      + '<div style="display:flex;gap:5px;align-items:stretch">'
      + '<div data-syv="agg-badges" style="display:flex;gap:4px;flex:1">' + updBadge('Core', coreOk) + updBadge('Supv', supOk) + updBadge('Addon', addonOk) + updBadge('HACS', hacsOk) + '</div>'
      + (hacsN != null && hacsN > 0 ? '<div style="font-size:9px;font-weight:700;color:#f97316;background:rgba(249,115,22,.1);border:1px solid rgba(249,115,22,.2);border-radius:6px;padding:3px 7px;white-space:nowrap;display:flex;align-items:center">' + hacsN + ' HACS</div>' : '')
      + (c.pk_cert ? '<div style="font-size:9px;font-weight:700;color:' + certCol + ';background:' + certCol + '12;border:1px solid ' + certCol + '30;border-radius:6px;padding:3px 7px;white-space:nowrap;display:flex;align-items:center">🔐 SSL</div>' : '')
      + '</div></div>';
    ...
    const togglesSection = '<div style="background:rgba(139,92,246,.06);border:1px solid rgba(139,92,246,.18);border-radius:11px;padding:7px 10px;position:relative;z-index:1">'
      ...
      + '<div style="font-size:9px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em">⚙ Automazioni</div>'
      + '<button data-sya="popup-notif" style="font-size:9px;color:rgba(255,255,255,.3);background:none;border:none;cursor:pointer;padding:0">notifiche ›</button>'
      + '</div>'
  ...
  function _patch(card, el) {
    ...
    function sv(k, t) { const e = el.querySelector('[data-syv="' + k + '"]'); if (e) e.textContent = t ?? '—'; }
    ...
    if (bdgRow) { const bdgs = statusBadges(cpuV, ramV, tmpV, l1, swpV); bdgRow.innerHTML = bdgs.map(b => '<div style="font-size:10px;font-weight:700;padding:3px 9px;border-radius:99px;background:' + b[1] + '18;border:1px solid ' + b[1] + '44;color:' + b[1] + '">' + b[0] + '</div>').join(''); }
    ...
    sv('en-oggi', fmtKwh(enOggi));
    sv('co-oggi', fmtEur(coOggi));
    sv('co-mese', fmtEur(coMese));
    sv('ha-uptime', haUptime || '—');
    sv('srv-uptime', uptimeText(h, c.pk_srv_uptime));
    sv('ent-count', entCount || '—');
    sv('last-bk', lastBk || '—');
  ...
  /* ── POPUPS ── */
  function slideUp(color) { return 'position:fixed;inset:0;z-index:100000;display:flex;align-items:flex-end;background:rgba(0,0,0,.6);backdrop-filter:blur(4px)'; }
  function popupShell(icon, iconCol, title, sub, contentHtml, closeId) {
    return '<style>@keyframes syUP{from{transform:translateY(100%)}to{transform:translateY(0)}}.pc{overflow-y:auto;scrollbar-width:none;-ms-overflow-style:none}.pc::-webkit-scrollbar{display:none}</style>'
      + '<div style="width:100%;max-height:72vh;display:flex;flex-direction:column;background:#0a0816;border:1px solid rgba(' + iconCol + ',.25);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -12px 60px rgba(0,0,0,.7);animation:syUP .22s cubic-bezier(.32,1.12,.56,1);overflow:hidden">'
        + '<div style="display:flex;align-items:center;gap:10px;padding:14px 16px 12px;border-bottom:1px solid rgba(255,255,255,.07);flex-shrink:0">'
          + '<div style="width:34px;height:34px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:16px;background:rgba(' + iconCol + ',.15);border:1px solid rgba(' + iconCol + ',.3);flex-shrink:0">' + icon + '</div>'
        ...
        + '</div>'
        + '<div class="pc" style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">' + contentHtml + '</div>'
      + '</div>';
  }
  function mkOv(html, closeId) {
    const ov = document.createElement('div');
    ov.style.cssText = slideUp();
    ov.innerHTML = html;
    document.body.appendChild(ov);
    const close = () => { try { document.body.removeChild(ov); } catch (e) {} };
    ov.querySelector('#' + closeId).addEventListener('click', close);
  ...
  function openEnergiaPopup(c) {
    ...
    function row(label, val, col) {
      return '<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.05)">'
        ...
        + '<span style="font-size:13px;font-weight:800;color:' + (col || '#fbbf24') + '">' + val + '</span></div>';
    }
    const pwV = num(S(h, c.pk_power));
    const content = '<div style="background:rgba(249,115,22,.1);border-radius:12px;padding:12px 14px;text-align:center"><div style="font-size:10px;font-weight:700;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Potenza Attuale</div><div style="font-size:28px;font-weight:900;color:#fb923c">' + (pwV == null ? '—' : pwV.toFixed(0) + ' W') + '</div></div>'
      + '<div><div style="font-size:10px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Consumo Energia</div>'
      + row('Oggi', fmtKwh(S(h, c.pk_en_oggi)), '#fdba74')
      + row('Questo mese', fmtKwh(S(h, c.pk_en_mese)), '#fdba74')
      + row('Questo anno', fmtKwh(S(h, c.pk_en_anno)), '#fdba74') + '</div>'
      + '<div><div style="font-size:10px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Costi (€)</div>'
      + row('Oggi', fmtEur(S(h, c.pk_co_oggi)), '#fbbf24')
      + row('Ieri', fmtEur(S(h, c.pk_co_ieri)), 'rgba(255,255,255,.5)')
      + row('Questo mese', fmtEur(S(h, c.pk_co_mese)), '#fbbf24')
      + row('Mese precedente', fmtEur(S(h, c.pk_co_mese_p)), 'rgba(255,255,255,.5)')
      + row('Questo anno', fmtEur(S(h, c.pk_co_anno)), '#fbbf24')
      + row('Anno precedente', fmtEur(S(h, c.pk_co_anno_p)), 'rgba(255,255,255,.5)') + '</div>';
    mkOv(popupShell('⚡', '249,115,22', 'Energia & Costi', 'Consumo server', content, 'en-close'), 'en-close');
  }

  function openAggPopup(c) {
    const h = H();
    function badge(label, isOk, text) {
      const col = isOk ? '#22c55e' : '#f97316';
      return '<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;border-radius:10px;background:' + col + '10;border:1px solid ' + col + '25;margin-bottom:6px">'
    ...
    const content = badge('Core HA', coreS === 'Aggiornato', coreS)
      + badge('Supervisor', supS === 'Aggiornato', supS)
      + badge('Add-on', addonS === 'Aggiornati', addonS)
      + badge('HACS', hacsS === 'Aggiornato', hacsS)
      + (hacsN && parseInt(hacsN) > 0 ? '<div style="padding:8px 12px;border-radius:10px;background:rgba(249,115,22,.1);border:1px solid rgba(249,115,22,.2);font-size:11px;color:#fdba74">' + hacsN + ' aggiornamenti HACS disponibili</div>' : '')
      + (certS != null ? '<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;border-radius:10px;background:rgba(56,189,248,.08);border:1px solid rgba(56,189,248,.2);font-size:11px"><span style="color:rgba(255,255,255,.7)">🔐 Certificato SSL</span><span style="font-weight:800;color:#38bdf8">' + certS + '</span></div>' : '');
    mkOv(popupShell('🔄', '56,189,248', 'Aggiornamenti', 'Stato sistema HA', content, 'ag-close'), 'ag-close');
  }

  function openNotifPopup() {
    const h = H();
    function togRow(id, label) {
      const on = isOn(h, id);
      return '<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.04)">'
        + '<span style="font-size:12px;color:rgba(255,255,255,.65)">' + label + '</span>'
        + '<button data-sya="toggle" data-eid="' + id + '" style="padding:3px 12px;border-radius:99px;border:1px solid ' + (on ? 'rgba(34,197,94,.5)' : 'rgba(255,255,255,.2)') + ';background:' + (on ? 'rgba(34,197,94,.15)' : 'rgba(255,255,255,.06)') + ';color:' + (on ? '#86efac' : 'rgba(255,255,255,.4)') + ';font-size:10px;font-weight:700;cursor:pointer">' + (on ? 'ON' : 'OFF') + '</button></div>';
    }
    function numRow(id, label) {
      const v = S(h, id);
      return '<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.04)">'
        + '<span style="font-size:12px;color:rgba(255,255,255,.55)">' + label + '</span>'
        + '<span style="font-size:12px;font-weight:800;color:#fbbf24">' + (v || '—') + '</span></div>';
    }
    function dtRow(id, label) {
      const v = S(h, id);
      return '<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.04)">'
        + '<span style="font-size:12px;color:rgba(255,255,255,.55)">' + label + '</span>'
        + '<span style="font-size:11px;font-weight:700;color:#a78bfa">' + (v || '—') + '</span></div>';
    }
    function sec(title) { return '<div style="font-size:9px;font-weight:800;color:rgba(255,255,255,.3);text-transform:uppercase;letter-spacing:.08em;margin-top:4px">' + title + '</div>'; }

    const content = sec('Automazioni principali')
      + togRow('input_boolean.on_off_alert_ha', 'Alert generale')
      + togRow('input_boolean.on_off_aggiornamenti_ha', 'Aggiornamenti HA')
      + sec('Notifiche sistema')
      + togRow('input_boolean.notifica_avvio_ha', 'Notifica avvio HA')
      + togRow('input_boolean.notifica_tempo_avvio_ha', 'Tempo avvio HA')
      + togRow('input_boolean.notifica_login_errato', 'Login errato')
      + togRow('input_boolean.notifica_certificato_scaduto', 'Certificato SSL scaduto')
      + sec('Notifiche aggiornamenti')
      + togRow('input_boolean.notifica_aggiornamenti_core', 'Core')
      + togRow('input_boolean.notifica_aggiornamenti_supervisor', 'Supervisor')
      + togRow('input_boolean.notifica_aggiornamenti_addon', 'Add-on')
      + togRow('input_boolean.notifica_aggiornamenti_hacs', 'HACS')
      + sec('Alert soglie')
      + togRow('input_boolean.alert_cpu', 'Alert CPU')
      + togRow('input_boolean.alert_ram', 'Alert RAM')
      + togRow('input_boolean.alert_temperatura', 'Alert Temperatura')
      + togRow('input_boolean.alert_disco', 'Alert Disco')
      + togRow('input_boolean.alert_db', 'Alert Database')
      + sec('Soglie configurate')
      + numRow('input_number.utilizzo_cpu', 'Soglia CPU (%)')
      + numRow('input_number.utilizzo_ram', 'Soglia RAM (%)')
      + numRow('input_number.temperatura_server', 'Soglia Temperatura (°C)')
      + numRow('input_number.temperatura_rack_on', 'Soglia Rack ON (°C)')
      + numRow('input_number.temperatura_rack_off', 'Soglia Rack OFF (°C)')
      + numRow('input_number.utilizzo_disco', 'Soglia Disco (%)')
      + sec('Orari programmati')
      + dtRow('input_datetime.orario_backup_homeassistant', 'Backup HA')
      + dtRow('input_datetime.orario_report_ha', 'Report HA')
      + dtRow('input_datetime.orario_riavvio_homeassistant', 'Riavvio HA')
      + dtRow('input_datetime.orario_riavvio_server', 'Riavvio Server')
      + dtRow('input_datetime.orario_notifiche_aggiornamenti_homeassistant', 'Notifica aggiornamenti');

    const ov = mkOv(popupShell('🔔', '139,92,246', 'Notifiche & Alert', 'Configurazione automazioni', content, 'nf-close'), 'nf-close');
    ov.addEventListener('click', function (e) {
      const t = e.target.closest('[data-sya="toggle"]');
      if (t) {
        const eid = t.dataset.eid, cur = isOn(H(), eid);
        callSvc('homeassistant', cur ? 'turn_off' : 'turn_on', { entity_id: eid });
        setTimeout(() => { t.textContent = !cur ? 'ON' : 'OFF'; t.style.background = !cur ? 'rgba(34,197,94,.15)' : 'rgba(255,255,255,.06)'; t.style.borderColor = !cur ? 'rgba(34,197,94,.5)' : 'rgba(255,255,255,.2)'; t.style.color = !cur ? '#86efac' : 'rgba(255,255,255,.4)'; }, 200);
      }
    });
  }
  ...
  function openHistPopup(entityId, label) {
    if (!entityId) return;
    const content = '<div id="sh-stats" style="display:flex;gap:8px"></div><div id="sh-chart" style="min-height:120px;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,.3);font-size:12px">Caricamento dati…</div>';
    mkOv(popupShell('📈', '251,191,36', label, 'Ultime 24 ore', content, 'sh-close'), 'sh-close');
    ...
    callApi('GET', 'history/period/' + start.toISOString() + '?filter_entity_id=' + entityId + '&end_time=' + now.toISOString()).then(function (data) {
      ...
      try {
        ...
        const pts = series.map(s => parseFloat(s.state)).filter(v => !isNaN(v));
      ...
      } catch (e) { chartEl.textContent = 'Errore nel caricamento'; }
    });
  }
  ...
  function openCfg(card, el) {
    ...
    const formHtml = '<div style="margin-bottom:10px"><label style="' + stLbl + '">Nome</label><input id="sy-name" type="text" value="' + (c.name || '').replace(/"/g, '&quot;') + '" placeholder="es. Mini PC, NAS, Server…" style="' + stBase + '"></div>'
      + '<div style="' + stSec + '">Utilizzi %</div>'
      + field('sy-cpu', 'CPU (%)', cf.cpu, 'sensor.processor_use')
      + field('sy-ram', 'RAM (%)', cf.ram, 'sensor.memory_use_percent')
      + field('sy-disk', 'Disco (%)', cf.disk, 'sensor.disk_use_percent')
      + field('sy-swap', 'Swap (%)', cf.swap, 'sensor.swap_use_percent')
      + field('sy-temp', 'Temperatura CPU (°C)', cf.temp, 'sensor.cpu_temperatura')
      + '<div style="' + stSec + '">Carico e avvio</div>'
      + field('sy-load1', 'Load 1m', cf.load1, 'sensor.load_1m')
      + field('sy-load5', 'Load 5m', cf.load5, 'sensor.load_5m')
      + field('sy-load15', 'Load 15m', cf.load15, 'sensor.load_15m')
      + field('sy-boot', 'Last boot (uptime server OS)', cf.boot, 'sensor.last_boot')
      ...
      + field('sy-memuse', 'RAM usata', cf.memuse, 'sensor.memory_use')
      + field('sy-memfree', 'RAM libera', cf.memfree, 'sensor.memory_free')
      + field('sy-diskuse', 'Disco usato', cf.diskuse, 'sensor.disk_use')
      + field('sy-diskfree', 'Disco libero', cf.diskfree, 'sensor.disk_free')
      + field('sy-swapuse', 'Swap usata', cf.swapuse, 'sensor.swap_use')
      + field('sy-diskr', 'Lettura disco', cf.diskr, 'sensor.disk_read_throughput')
      + field('sy-diskw', 'Scrittura disco', cf.diskw, 'sensor.disk_write_throughput')
      + '<div style="' + stSecPk + '">⚡ PKG — Energia</div>'
      + field('sy-pk-power', 'Potenza attuale (W)', cf.pk_power, 'sensor.sensore_potenza_server_w')
      ...
      + field('sy-pk-co-mese-p', 'Costo mese precedente (€)', cf.pk_co_mese_p, 'sensor.costo_consumo_mese_precedente_server')
      + field('sy-pk-co-anno', 'Costo anno (€)', cf.pk_co_anno, 'sensor.costo_consumo_annuale_server')
      + field('sy-pk-co-anno-p', 'Costo anno precedente (€)', cf.pk_co_anno_p, 'sensor.costo_consumo_anno_precedente_server')
      ...
      + field('sy-pk-ha-start', 'Avvio HA (testo log)', cf.pk_ha_start, 'sensor.homeassistant_start')
    ...
    ov.innerHTML = '<style>@keyframes sySlideUp{from{transform:translateY(100%)}to{transform:translateY(0)}}@media(max-width:600px){.frk-cfg-cols{flex-direction:column!important;overflow-y:auto!important;overflow-x:hidden!important}.frk-form-col{width:100%!important;border-right:none!important;border-bottom:1px solid rgba(255,255,255,.07)!important;overflow-y:visible!important;flex-shrink:0!important}.frk-prev-col{min-width:0!important;overflow-y:visible!important}}</style>'
      + '<div style="width:100%;max-height:92vh;display:flex;flex-direction:column;background:#0a0816;border:1px solid rgba(139,92,246,.32);border-bottom:none;border-radius:20px 20px 0 0;box-shadow:0 -12px 60px rgba(0,0,0,.8);color:#fff;overflow:hidden;animation:sySlideUp .22s cubic-bezier(.32,1.12,.56,1)">'
        ...
        + '<div class="frk-cfg-cols" style="display:flex;flex:1;overflow:hidden;min-height:0">'
          + '<div class="frk-form-col" style="width:420px;flex-shrink:0;overflow-y:auto;padding:14px 16px;border-right:1px solid rgba(255,255,255,.07);scrollbar-width:none">' + formHtml + '</div>'
    ...
    allFieldIds.forEach(function (fid) {
      ...
      function showDrop() {
        ...
        var hits = (q ? allIds.filter(id => id.toLowerCase().includes(q) || ((states[id] && states[id].attributes && states[id].attributes.friendly_name || '').toLowerCase().includes(q))) : allIds).slice(0, 50);
        if (!hits.length) { drop.style.display = 'none'; return; }
        drop.style.display = 'block';
        drop.innerHTML = hits.map(id => { var fn = (states[id] && states[id].attributes && states[id].attributes.friendly_name) || ''; return '<div data-pick="' + id + '" style="padding:5px 11px;cursor:pointer;font-size:11px;font-family:monospace;border-bottom:1px solid rgba(255,255,255,.04)"><span style="color:#e2e8f0">' + id + '</span>' + (fn ? '<span style="color:#475569;margin-left:7px;font-family:system-ui;font-size:10px">' + fn + '</span>' : '') + '</div>'; }).join('');
    ...
    ov.querySelector('#sy-save').addEventListener('click', function () {
      ...
      save(card, {
        name: g('sy-name'),
        cpu: g('sy-cpu'), ram: g('sy-ram'), disk: g('sy-disk'), swap: g('sy-swap'), temp: g('sy-temp'),
  ...
  var CARD = {
    id: 'system-card', name: 'Mini-PC', icon: '🖥️', version: '4.7',
    desc: 'Mini-PC / Server: CPU/RAM/Disco/Swap ring, energia & costi PKG, aggiornamenti HA, toggle automazioni, notifiche & alert, grafici storici. Sensori PKG configurabili.',
    colSpan: 2, rowSpan: 4,
