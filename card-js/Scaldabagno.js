...
(function () {
  ...
  function openEnergiaPopup(c) {
    ...
    const content = '<div style="background:rgba(249,115,22,.1);border:1px solid rgba(249,115,22,.2);border-radius:12px;padding:12px 14px;text-align:center;margin-bottom:12px">'
      ...
      + (tempSet != null ? '<div style="font-size:11px;color:#fff;margin-top:4px">Setpoint: ' + tempSet.toFixed(0) + ' °C</div>' : '')
      + '</div>'
  ...
  function _openWizard(hass, onDone, _tpl, opts) {
    ...
    function renderWiz() {
      sr.innerHTML = '<style>'
        ...
        + '<div><div class="wd-tit">' + (isUpdate ? 'Aggiorna PKG Scaldabagno' : 'Installa PKG Scaldabagno') + '</div><div class="wd-sub">frarik_scaldabagno.yaml → config/packages/</div></div>'
