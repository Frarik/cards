...
(function () {
  ...
  function openImpostazioniHAPopup(c) {
    ...
    dToggle('input_boolean.asciugatrice_notify_push',   '📱 Push');
    dToggle('input_boolean.asciugatrice_notify_google', '🔊 Google');
    dToggle('input_boolean.asciugatrice_notify_alexa',  '🗣 Alexa');
    dTime('input_datetime.asciugatrice_notifiche_inizio', '⏰ Orario inizio notifiche');
    dTime('input_datetime.asciugatrice_notifiche_fine',   '⏰ Orario fine notifiche');

    dSec('🔌 Elettrodomestico');
    dToggle('input_boolean.asciugatrice_switch', 'Switch presa');
    dNum('input_number.frarik_asciugatrice_soglia_w',          'Soglia lavoro',  'W',   0, 5000, 1);
    dNum('input_number.asciugatrice_tempo_innesco_m',   'Delay spegnimento', 'min', 0, 60, 1);
    dNum('input_number.asciugatrice_avvio_ritardato_s', 'Delay riavvio',  's',   0, 300, 1);

    dSec('⏰ Spegnimento automatico');
    dToggle('automation.asciugatrice_off_automatico', 'Auto OFF abilitato');
    dTime('input_datetime.asciugatrice_off', 'Orario spegnimento');

    dSec('📝 Personalizzazione');
    dText('input_text.asciugatrice_nome',      'Nome elettrodomestico');
    dText('input_text.asciugatrice_messaggio', 'Messaggio notifica');
    dNum('input_number.costo_energia',  'Costo energia', '€/kWh', 0, 2, 0.001);
    dInfo('Ultimo reset contatori', ss('input_text.asciugatrice_data_reset'));
    ...
    const ov = mkOv(popShell('⚙', '100,116,139', 'Impostazioni', c.name || 'Asciugatrice', closeId, swCss + rows.join('') + resetBtn), closeId);
