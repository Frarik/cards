...
(function () {
  ...
  function render(card) {
    ...
    return css
      ...
      + '<div class="fc-hdr-tit">' + (c.name || 'Microonde') + '</div>'
  ...
  function openEnergiaPopup(c) {
    ...
    const content = '<div style="background:rgba(56,189,248,.1);border-radius:12px;padding:12px 14px;text-align:center;margin-bottom:12px">'
      ...
      + row('Mese precedente', fmtEur(Attr(h, ton, 'costo_mese_prec_microonde')), '#fff')
      + row('Questo anno', fmtEur(Attr(h, ton, 'costo_anno_microonde')), '#7dd3fc')
      + row('Anno precedente', fmtEur(Attr(h, ton, 'costo_anno_prec_microonde')), '#fff');
  ...
  function openCicliPopup(c) {
    ...
    DAYS.forEach(function (d, i) {
      ...
      const cicli = isToday ? cleanVal(S(h, c.pk_cicli_oggi))      : cleanVal(S(h, 'input_text.' + d + '_microonde_cicli'));
      const tempo = isToday ? cleanVal(Attr(h, ton, 'Oggi'))        : cleanVal(S(h, 'input_text.' + d + '_microonde_tempo'));
  ...
  function openImpostazioniHAPopup(c) {
    ...
    dTime('input_datetime.frarik_microonde_notifiche_inizio', '⏰ Orario inizio notifiche');
    dTime('input_datetime.frarik_microonde_notifiche_fine',   '⏰ Orario fine notifiche');
    ...
    dTime('input_datetime.frarik_microonde_off', 'Orario spegnimento');
    ...
    const ov = mkOv(popShell('⚙', '100,116,139', 'Impostazioni', c.name || 'Microonde', closeId, swCss + rows.join('') + saveBtn + resetBtn), closeId);
