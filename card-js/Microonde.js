...
(function () {
  ...
  function openImpostazioniHAPopup(c) {
    ...
    dToggle('input_boolean.microonde_notify_push',   '📱 Push');
    dToggle('input_boolean.microonde_notify_google', '🔊 Google');
    dToggle('input_boolean.microonde_notify_alexa',  '🗣 Alexa');
    dTime('input_datetime.microonde_notifiche_inizio', '⏰ Orario inizio notifiche');
    dTime('input_datetime.microonde_notifiche_fine',   '⏰ Orario fine notifiche');
    ...
    dToggle('automation.microonde_off_automatico', 'Auto OFF abilitato');
    dTime('input_datetime.microonde_off', 'Orario spegnimento');

    dSec('📝 Personalizzazione');
    dText('input_text.microonde_nome',      'Nome elettrodomestico');
    dText('input_text.microonde_messaggio', 'Messaggio notifica');
    dNum('input_number.costo_energia',  'Costo energia', '€/kWh', 0, 2, 0.001);
    dInfo('Ultimo reset contatori', ss('input_text.microonde_data_reset'));
    ...
    const ov = mkOv(popShell('⚙', '100,116,139', 'Impostazioni', c.name || 'Microonde', closeId, swCss + rows.join('') + resetBtn), closeId);
